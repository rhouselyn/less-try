    ...
    >
      {hasWrongItems && onSkipReview && (
        <motion.button
          ...
          className="absolute -top-2 left-0 z-10 flex items-center gap-1.5 btn-ghost text-xs border-[3px] border-[#1a1a2e] hover:border-[#e63946]"
        >
          <X className="w-3 h-3" />
          {t.skipReview || '不想复习了'}
        </motion.button>
      )}
      <div className="bg-white border-[3px] border-[#1a1a2e] rounded-2xl p-12 shadow-cel text-center">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 15, delay: 0.2 }}
          className="w-24 h-24 bg-[#fef3e2] rounded-full border-[3px] border-[#1a1a2e] flex items-center justify-center mx-auto mb-8"
        >
          <Trophy className="w-12 h-12 text-[#e63946]" />
        </motion.div>
        ...
        <motion.h2
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3 }}
          className="text-3xl font-bold font-display text-[#1a1a2e] mb-4"
        >
          🎉 {t.unitComplete || '单元完成！'}
        </motion.h2>
        ...
        <motion.p
          ...
          className="text-base text-[#2d2d4a] mb-8"
        >
          {errorCount === 0
        ...
        {hasWrongItems && (
          <motion.div
            ...
            className="mb-8 p-4 bg-[#fef3e2] border-[3px] border-[#e63946] rounded-xl"
          >
            <div className="flex items-center gap-2 justify-center mb-2">
              <RotateCcw className="w-4 h-4 text-[#e63946]" />
              <span className="text-[#1a1a2e] font-medium">{t.wrongItemReview || '错题复习'}</span>
            </div>
            <p className="text-sm text-[#2d2d4a]">
        ...
        >
          ...
          <span className="text-[#2d2d4a] text-sm">
