...
- **翻译题问题**：翻译题使用原文而不是母语（翻译后的文字）
- **句子翻译卡片顺序**：句子翻译卡片的顺序搞反了，应该是先original后译文
- **词性显示**：单词表漏了词性显示
...
### 3.1 修复句子翻译显示问题
- 修改 `SentenceDetail.jsx`，确保句子翻译卡片的顺序正确：先original后译文
...
### 3.3 确保单词表显示词性
- 确保 `main.py` 中提取词汇时包含词性信息
- 确保前端 `DictionaryStep.jsx` 正确显示词性

### 3.4 修复翻译题问题
- 修改 `generate_sentence_quiz` 方法，使用母语（翻译后的文字）而不是原文
...
### 步骤 1：修复 SentenceDetail.jsx 中的显示顺序
- 确保句子翻译卡片的顺序正确：先original后译文

### 步骤 2：确保单词表显示词性
- 检查 `main.py` 中提取词汇时是否包含词性信息
- 确保前端 `DictionaryStep.jsx` 正确显示词性

### 步骤 3：修复翻译题使用原文的问题
- 修改 `generate_sentence_quiz` 方法，使用母语（翻译后的文字）而不是原文

### 步骤 4：验证 process_text_with_dictionary 方法
...
- 句子翻译板块中显示正确的顺序：先original后译文
- 单词表中正确显示词性
- 单词表中的例句使用正确的语言
- 翻译题使用母语（翻译后的文字）而不是原文
- 翻译题在学习完足够单词后正确生成
- 所有功能正常运行
