...
- **FR-2**: 词汇提取和去重 - 从每个句子中提取单词并全局去重，只针对单词去重，取token翻译的并集作为释义集合
- **FR-3**: Tool Schema - 定义 `generate_dictionary` 工具，强制输出 JSON 格式，用于答题时再现场生成
- **FR-4**: 结构化输出 - `split_and_translate` 输出包含：
  - 分词结果（tokens）：每个token包含文字、对应翻译、音标、形态
  - 翻译结果：按token分词的翻译
  - 语法讲解：对整个句子的语法分析
- **FR-5**: 前端流程 - 点击生成按钮后跳转到单词表页面，直接使用 `split_and_translate` 的输出生成字典
- **FR-6**: 双栏字典 UI - 左侧单词列表（随机排列），右侧详细信息
- **FR-7**: 音标处理 - 如果音标有不同（可能的生成错误），选出现最多的，一样多就选第一个
- **FR-8**: 变体类型标注 - 单词的variants在变种前面需要说明类型（动词等）
...
- **NFR-5**: 前端交互 - 流畅的动画效果和过渡，提升用户体验
- **NFR-6**: 品牌一致性 - 严格遵循 Anthropic 品牌指南，确保视觉统一性
