                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "translation_phrases": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "将 tokenized_translation 拆分为独立片段，用于翻译排序练习。必须至少拆分为2个片段！【拆分原则】1.优先按目标语言的自然词边界拆成单个词或短词组；2.【极其重要】固定搭配、习语、短语动词、惯用表达必须作为整体不拆分——如果拆分后各部分无法独立表达整体含义，则必须保持为一个片段。这是所有语言的通用原则；3.虚词（的、了、地等）可以与相邻词合并；4.每个片段不能是单个无意义虚词。【极其重要】所有片段按顺序拼接后必须等于 tokenized_translation 的内容（去除标点差异后），不能遗漏或增加内容"
                        },
                        "grammar_explanation": {
                            "type": "string",
...
- translation_phrases: 将 tokenized_translation 拆分为独立片段，用于翻译排序练习。必须至少拆分为2个片段！【拆分原则】1.优先按目标语言的自然词边界拆成单个词或短词组；2.【极其重要】固定搭配、习语、短语动词、惯用表达必须作为整体不拆分——如果拆分后各部分无法独立表达整体含义，则必须保持为一个片段。这是所有语言的通用原则；3.虚词可以与相邻词合并；4.每个片段不能是单个无意义虚词。【极其重要】所有片段按顺序拼接后必须等于 tokenized_translation 的内容（去除标点差异后），不能遗漏或增加内容
