...
4. 【极其重要·默认按词切分，极少数情况才合并】对于用空格分隔词语的语言，空格就是词边界，每个空格分隔的单元就是一个独立的 token。不要将多个空格分隔的词合并为一个 token，除非满足以下严格条件：(1) 整体含义完全无法从各组成部分的字面含义推导出来——即如果去掉其中任何一个词，整体含义就完全改变或无法理解；(2) 该表达在短语词典中作为不可分割的独立条目存在。常见的语法组合（如不定式结构、介宾搭配、动宾搭配、主谓搭配、修饰关系等）绝对不是固定搭配，必须拆分为独立 token。每个词只要能在词典中查到独立释义，就必须作为独立 token。过度合并是比漏分更严重的错误——宁可多分也不要误合。
