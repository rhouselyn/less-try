...
5. 【极其重要·标点禁令】text 字段绝对禁止包含任何标点符号（句号、逗号、问号、感叹号、分号、冒号、省略号等）。标点不属于任何 token，它们不属于任何词。但连字符(-)和撇号(')如果在该语言中是词的内部组成部分则必须保留（如法语 l'homme、俄语 по-русски）。例如："Здравей!" 必须拆分为 text="Здравей"，感叹号不属于任何 token；"Hello," 必须拆分为 text="Hello"，逗号不属于任何 token。
