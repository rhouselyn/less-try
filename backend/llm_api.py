...
async def call_llm_with_rotation(messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.0, max_tokens: int = 4096):
...
async def detect_language(text: str) -> str:
    ...
    result = await call_llm_with_rotation(messages, temperature=0.0, max_tokens=32)
...
class LLMClient:
    ...
    async def call_with_rotation(cls, messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.0, max_tokens: int = 4096):
        return await call_llm_with_rotation(messages, tools=tools, temperature=temperature, max_tokens=max_tokens)
...
"""
        ...
        response = await call_llm_with_rotation(messages, [tool_def], temperature=temperature, max_tokens=16384)
...
"""
        ...
        response = await call_llm_with_rotation(messages, [tool_def], temperature=0.0, max_tokens=16384)
        try:
            for choice in response["choices"]:
                if "tool_calls" in choice["message"]:
                    tool_call = choice["message"]["tool_calls"][0]
