...
function FrogLogo({ size = 40 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <ellipse cx="50" cy="58" rx="38" ry="32" fill="var(--color-success)" stroke="var(--color-dark)" strokeWidth="3" />
      <ellipse cx="50" cy="55" rx="34" ry="28" fill="#5cb8ab" />
      <circle cx="34" cy="38" r="16" fill="var(--color-success)" stroke="var(--color-dark)" strokeWidth="3" />
      <circle cx="66" cy="38" r="16" fill="var(--color-success)" stroke="var(--color-dark)" strokeWidth="3" />
      <circle cx="34" cy="38" r="13" fill="#fff" stroke="var(--color-dark)" strokeWidth="2" />
      <circle cx="66" cy="38" r="13" fill="#fff" stroke="var(--color-dark)" strokeWidth="2" />
      <circle cx="36" cy="37" r="6" fill="var(--color-dark)" />
      <circle cx="68" cy="37" r="6" fill="var(--color-dark)" />
      <circle cx="38" cy="35" r="2" fill="#fff" />
      <circle cx="70" cy="35" r="2" fill="#fff" />
      <ellipse cx="50" cy="62" rx="18" ry="8" fill="#ffd166" />
      <path d="M38 60 Q50 70 62 60" stroke="var(--color-dark)" strokeWidth="3" fill="none" strokeLinecap="round" />
    </svg>
  )
}
              ...
              {wordListLang ? (
                ...
                <>
                  <motion.button
                    ...
                    className="absolute top-3 right-4 p-2 text-[var(--color-dark)] hover:text-[var(--color-accent)] hover:bg-[var(--color-warning)]/20 rounded-[var(--radius-sm)] transition-colors z-10 border-[var(--border-width)] border-[var(--border-color)] bg-[var(--color-card)] shadow-[var(--shadow-sm)]"
                  >
                    <Settings className="w-5 h-5" />
                  </motion.button>
                  {translatingUI && (
                    <div className="absolute inset-0 bg-[var(--color-bg)]/90 backdrop-blur-sm z-20 flex items-center justify-center">
                      <div className="flex items-center gap-3 bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-sm)] px-6 py-4 shadow-[var(--shadow)]">
                        <Loader2 className="w-5 h-5 animate-spin text-[var(--color-accent)]" />
                        <span className="text-sm text-[var(--color-dark)] font-black uppercase">{
