...
function WordDetailCard({ word, sourceLang, detailLoading, t }) {
  return (
    ...
    >
      <div className="px-5 pb-5 pt-3 border-t border-stone-200 bg-canvas-50">
        {detailLoading ? (
          <div className="pt-4 flex flex-col items-center justify-center gap-3">
            <Loader2 className="w-6 h-6 animate-spin text-cadmium-500" />
            <p className="text-[13px] text-umber-500">{t.generatingWordDetails || '正在生成单词详解...'}</p>
          </div>
        ) : (
          <>
            <div className="mb-3">
              <h3 className="text-[11px] font-semibold text-cadmium-700 uppercase tracking-[0.12em] mb-1.5 flex items-center gap-1.5">
                <Brain className="w-3.5 h-3.5 text-cadmium-600" />
                {t.definition || '释义'}
              </h3>
              <p className="text-[14px] text-umber-700 leading-relaxed">
            ...
            {word.variants_detail && word.variants_detail.length > 0 && (
              <div className="mb-3">
                <h3 className="text-[11px] font-semibold text-cadmium-700 uppercase tracking-[0.12em] mb-1.5 flex items-center gap-1.5">
                  <GitBranch className="w-3.5 h-3.5 text-cadmium-600" />
                  {t.variants || '词形变化'}
                </h3>
                <div className="flex flex-wrap gap-x-4 gap-y-2">
                  {word.variants_detail.map((variant, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <span className="px-2 py-1 bg-cadmium-100 text-cadmium-700 rounded text-[12px] font-medium">
                        {variant.type}
                      </span>
                      <span className="text-umber-700 text-[14px]">{variant.form}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {word.examples && word.examples.length > 0 && (
              <div className="mb-3">
                <h3 className="text-[11px] font-semibold text-cadmium-700 uppercase tracking-[0.12em] mb-1.5 flex items-center gap-1.5">
                  <BookText className="w-3.5 h-3.5 text-cadmium-600" />
                  {t.examples || '例句'}
                </h3>
                  ...
                  {word.examples.slice(0, 3).map((ex, i) => (
                    <div key={i} className="border-l-3 border-cadmium-300 pl-3 py-1">
                      <div className="flex items-start gap-2">
                        <p className="text-umber-800 text-[14px] leading-snug flex-1">{ex.sentence}</p>
                        {ex.sentence && (
                          <button
                            onClick={(e) => { e.stopPropagation(); speakText(ex.sentence, sourceLang) }}
                            className="p-1 text-cadmium-500 hover:text-cadmium-700 hover:bg-cadmium-100 rounded transition-colors shrink-0"
                          >
                      ...
                      {ex.translation && (
                        <p className="text-umber-600 text-[13px] leading-snug mt-0.5">{ex.translation}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {word.memory_hint && (
              <div className="mb-1">
                <h3 className="text-[11px] font-semibold text-cadmium-700 uppercase tracking-[0.12em] mb-1.5 flex items-center gap-1.5">
                  <Lightbulb className="w-3.5 h-3.5 text-cadmium-600" />
                  {t.memoryHint || '记忆辅助'}
                </h3>
                <p className="text-[14px] text-umber-700 leading-relaxed bg-cadmium-100/70 px-4 py-3 rounded-xl border border-cadmium-200">
                  {word.memory_hint}
                </p>
              </div>
            )}
...
function WordListPanel({ sourceLang, t, onBack, pageSize = 50 }) {
    ...
    return (
      <div className="flex h-full">
        {allLetterIndex.length > 1 && !searchQuery && (
          <div className="flex flex-col items-center gap-px py-1 border-r border-stone-200 bg-canvas-50 shrink-0 w-6">
            {allLetterIndex.map(letter => {
              ...
              return (
                <button
                  ...
                  className={`w-5 h-5 flex items-center justify-center text-[8px] font-semibold rounded transition-colors leading-tight ${
                    onCurrentPage
                      ? 'text-umber-700 hover:text-cadmium-600 hover:bg-cadmium-100'
                      : 'text-stone-400 hover:text-cadmium-600 hover:bg-cadmium-50'
                  }`}
                >
                  {letter}
                </button>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto" ref={listRef}>
          {groupedWords.map(([letter, groupWords], groupIdx) => (
            <div key={letter} id={`letter-${letter}`}>
              <div className="sticky top-0 z-10 px-5 py-2 bg-cadmium-50 backdrop-blur-sm border-b border-cadmium-200">
              ...
              {groupWords.map((word, wordIdx) => (
                <div key={word.word} ref={el => { wordRefs.current[word.word] = el }} className={wordIdx % 2 === 0 ? 'bg-white' : 'bg-canvas-50'}>
                  <div
                    role="button"
                    tabIndex={0}
                    onClick={() => handleWordClick(word.word)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleWordClick(word.word) }}
                    className="w-full flex items-center gap-3 px-5 py-3 hover:bg-cadmium-100/50 transition-colors text-left cursor-pointer"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      {word.meaning && (
                        <p className={`text-sm text-umber-600 mt-0.5 truncate ${displayMode === 1 && expandedWord !== word.word ? 'invisible' : ''}`}>{meaningOverrides[word.word] || word.meaning}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {expandedWord === word.word && detailLoading[word.word] ? (
                        <Loader2 className="w-4 h-4 animate-spin text-cadmium-500" />
  ...
  if (onBack) {
    return (
      <div className="h-full flex flex-col bg-white rounded-2xl border border-stone-400 shadow-impasto overflow-hidden min-h-0">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-300 bg-gradient-to-r from-cadmium-100 to-canvas-100">
          <div className="flex items-center gap-2.5">
            <BookOpen className={`w-5 h-5 cursor-pointer transition-colors ${displayMode !== 0 ? 'text-cadmium-600' : 'text-cadmium-600 hover:text-cadmium-700'}`} onClick={() => setDisplayMode(v => (v + 1) % 3)} title={displayMode === 0 ? (t.showAll || '显示全部') : displayMode === 1 ? (t.hideMeaning || '隐藏释义') : (t.hideWord || '隐藏单词')} />
            <span className="text-lg font-semibold font-display text-umber-800">{t.vocabOverview || '词汇总览'}</span>
          ...
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-umber-600 hover:text-umber-800 hover:bg-white rounded-lg transition-colors"
          >
        ...
        <div className="px-4 py-3 border-b border-stone-200 bg-canvas-50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2.5 text-sm bg-white border border-stone-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-cadmium-400 focus:border-cadmium-400 placeholder:text-umber-400 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-umber-400 hover:text-umber-700 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            )}
  ...
  return (
    <div className="mt-6">
      <motion.button
        ...
        className="w-full flex items-center justify-between px-5 py-4 bg-white rounded-2xl border border-stone-400 hover:border-cadmium-300 transition-all shadow-impasto"
      >
        <div className="flex items-center gap-2.5">
        ...
        >
          <ChevronDown className="w-5 h-5 text-umber-500" />
        </motion.div>
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="overflow-hidden"
          >
            <div className="mt-3 bg-white rounded-2xl border border-stone-400 shadow-impasto">
              <div className="p-4 border-b border-stone-200 bg-canvas-50">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-umber-400" />
                  <input
                    type="text"
