...
function WordDetailCard({ word, sourceLang, detailLoading, t }) {
  return (
    ...
    >
      <div className="px-5 pb-4 pt-3 border-t border-stone-200 bg-gradient-to-b from-cadmium-50/50 to-white">
        {detailLoading ? (
          <div className="pt-4 flex flex-col items-center justify-center gap-3">
            <Loader2 className="w-5 h-5 animate-spin text-cadmium-500" />
            <p className="text-[12px] text-umber-500">{t.generatingWordDetails || '正在生成单词详解...'}</p>
          </div>
        ) : (
          <>
            <div className="mb-3">
              <h3 className="text-[11px] font-semibold text-cadmium-700 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                <Brain className="w-3 h-3" />
                {t.definition || '释义'}
              </h3>
              <p className="text-[13px] text-umber-700 leading-relaxed bg-white/80 px-3 py-2.5 rounded-lg border border-cadmium-100">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1.5">
                  {word.variants_detail.map((variant, index) => (
                    <div key={index} className="flex items-center gap-1.5">
                      <span className="px-2 py-0.5 bg-cadmium-100 text-cadmium-700 rounded text-[11px] font-medium">
                        {variant.type}
                      </span>
                      <span className="text-umber-700 text-[13px]">{variant.form}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-3">
                ...
                <div className="space-y-2">
                  {word.examples.slice(0, 3).map((ex, i) => (
                    <div key={i} className="border-l-3 border-cadmium-300 pl-3 py-1">
                      <div className="flex items-start gap-1.5">
                        <p className="text-umber-800 text-[13px] leading-snug flex-1">{ex.sentence}</p>
                        {ex.sentence && (
                          <button
                            onClick={(e) => { e.stopPropagation(); speakText(ex.sentence, sourceLang) }}
                            className="p-0.5 text-cadmium-500 hover:text-cadmium-700 hover:bg-cadmium-100 rounded transition-colors shrink-0"
                          >
                      ...
                      {ex.translation && (
                        <p className="text-umber-500 text-[11px] leading-snug mt-0.5">{ex.translation}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {word.memory_hint && (
              <div className="mb-2">
                ...
                <p className="text-[13px] text-umber-700 leading-relaxed bg-cadmium-100/60 px-3 py-2.5 rounded-lg border border-cadmium-200">
                  {word.memory_hint}
                </p>
              </div>
            )}
...
function WordListPanel({ sourceLang, t, onBack, pageSize = 50 }) {
    ...
    return (
      <div className="flex items-center justify-center gap-1 py-2 border-t border-stone-300 bg-canvas-100">
        ...
        <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
          className={`p-1 rounded transition-colors ${page >= totalPages ? 'text-stone-200 cursor-not-allowed' : 'text-umber-400 hover:text-umber-600 hover:bg-canvas-100'}`}>
    ...
    if (loading) {
      return (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="w-6 h-6 animate-spin text-cadmium-500" />
          <span className="ml-2 text-sm text-umber-500">{t.loadingVocab || '加载单词表...'}</span>
        </div>
      )
    }
    ...
    return (
      <div className="flex h-full">
        {allLetterIndex.length > 1 && !searchQuery && (
          <div className="flex flex-col items-center gap-px py-1 border-r border-stone-300 bg-gradient-to-b from-cadmium-50 to-canvas-50 shrink-0 w-5">
        ...
        <div className="flex-1 min-w-0 overflow-y-auto" ref={listRef}>
          {groupedWords.map(([letter, groupWords]) => (
            <div key={letter} id={`letter-${letter}`}>
              <div className="sticky top-0 z-10 px-5 py-2 bg-gradient-to-r from-cadmium-100/95 to-cadmium-50/95 backdrop-blur-md border-b border-stone-300">
                <span className="text-xs font-bold text-cadmium-700 tracking-wider">{letter}</span>
                <span className="text-[10px] text-umber-500 ml-1.5 font-medium">{groupWords.length}</span>
              </div>
              {groupWords.map((word, index) => {
                const isEven = index % 2 === 0
                return (
                <div key={word.word} ref={el => { wordRefs.current[word.word] = el }} className={`${isEven ? 'bg-white' : 'bg-cadmium-50/50'}`}>
                  <div
                    role="button"
                    tabIndex={0}
                    onClick={() => handleWordClick(word.word)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleWordClick(word.word) }}
                    className="w-full flex items-center gap-3 px-5 py-3 hover:bg-cadmium-100/60 transition-colors text-left cursor-pointer"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {word.ipa && (
                          <span className={`text-xs text-umber-500 ipa-font ${displayMode === 2 && expandedWord !== word.word ? 'invisible' : ''}`}>
                            {word.ipa.startsWith('/') ? word.ipa : `/${word.ipa}/`}
                          </span>
                        )}
                        {word.part_of_speech && (
                          <span className="text-[10px] px-1.5 py-0.5 bg-cadmium-100 text-cadmium-700 rounded font-medium">
                            {word.part_of_speech}
                          </span>
                        )}
                      </div>
                      {word.meaning && (
                        <p className={`text-xs text-umber-600 mt-0.5 truncate ${displayMode === 1 && expandedWord !== word.word ? 'invisible' : ''}`}>{meaningOverrides[word.word] || word.meaning}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {expandedWord === word.word && detailLoading[word.word] ? (
                        <Loader2 className="w-3.5 h-3.5 animate-spin text-cadmium-500" />
                      ) : expandedWord === word.word ? (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleRegenerateWord(word.word) }}
                          className="p-1.5 text-umber-400 hover:text-cadmium-600 hover:bg-cadmium-100 rounded-full transition-colors"
  ...
  if (onBack) {
    return (
      <div className="h-full flex flex-col bg-gradient-to-br from-canvas-50 to-canvas-100 rounded-2xl border border-stone-300 shadow-impasto overflow-hidden min-h-0">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-stone-300 bg-gradient-to-r from-cadmium-100 to-canvas-100">
          <div className="flex items-center gap-2.5">
            <BookOpen className={`w-5 h-5 cursor-pointer transition-colors ${displayMode !== 0 ? 'text-cadmium-700' : 'text-cadmium-600 hover:text-cadmium-700'}`} onClick={() => setDisplayMode(v => (v + 1) % 3)} title={displayMode === 0 ? (t.showAll || '显示全部') : displayMode === 1 ? (t.hideMeaning || '隐藏释义') : (t.hideWord || '隐藏单词')} />
            <span className="text-base font-semibold font-display text-umber-800">{t.vocabOverview || '词汇总览'}</span>
            {!loading && words.length > 0 && (
              <span className="text-xs text-cadmium-700 bg-cadmium-200 px-2.5 py-0.5 rounded-full font-medium">{words.length} {t.wordCount || '词'}</span>
            )}
          </div>
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-umber-600 hover:text-umber-800 hover:bg-canvas-50 rounded-lg transition-colors"
          >
        ...
        <div className="px-4 py-3 border-b border-stone-300 bg-canvas-50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2.5 text-sm bg-white border border-stone-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-cadmium-400 focus:border-cadmium-400 placeholder-umber-400 transition-all shadow-sm"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 text-umber-400 hover:text-umber-700 transition-colors"
              >
  ...
  return (
    <div className="mt-6">
      <motion.button
        ...
        className="w-full flex items-center justify-between px-5 py-4 bg-gradient-to-r from-cadmium-100 to-canvas-100 rounded-2xl border border-stone-300 hover:border-cadmium-400 transition-all shadow-impasto-sm"
      >
        <div className="flex items-center gap-2.5">
          <BookOpen className="w-4 h-4 text-cadmium-700" />
          <span className="text-sm font-semibold font-display text-umber-800">{t.vocabOverview || '词汇总览'}</span>
          {!isOpen && words.length > 0 && (
            <span className="text-xs text-cadmium-700 bg-cadmium-200 px-2 py-0.5 rounded-full font-medium">{words.length} {t.wordCount || '词'}</span>
          )}
        </div>
        <motion.div
        ...
        >
          <ChevronDown className="w-4 h-4 text-umber-600" />
        </motion.div>
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="overflow-hidden"
          >
            <div className="mt-3 bg-white rounded-2xl border border-stone-300 shadow-impasto overflow-hidden">
              <div className="p-4 border-b border-stone-300 bg-gradient-to-r from-cadmium-50 to-canvas-50">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-umber-400" />
                  <input
                    type="text"
                ...
                <div className="flex items-center justify-between mt-3">
                  <span className="text-xs text-umber-500 font-medium">
