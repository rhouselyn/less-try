    ...
    >
      <div className="px-5 pb-4 pt-2 border-t-[3px] border-[#1a1a2e]">
        {detailLoading ? (
          ...
          <>
            <div className="mb-2">
              <h3 className="label-warm text-[10px] font-black text-[#4a4a6a] uppercase tracking-[0.12em] mb-0.5 flex items-center gap-1">
            ...
            {word.variants_detail && word.variants_detail.length > 0 && (
              <div className="mb-2">
                <h3 className="label-warm text-[10px] font-black text-[#4a4a6a] uppercase tracking-[0.12em] mb-0.5 flex items-center gap-1">
                  <GitBranch className="w-3 h-3 text-[#ff006e]" />
                  {t.variants || '词形变化'}
                </h3>
                <div className="flex flex-wrap gap-x-3 gap-y-1">
                  {word.variants_detail.map((variant, index) => (
                    <div key={index} className="flex items-center gap-1.5">
                      <span className="badge-ochre px-1.5 py-0.5 bg-[#fff0f5] text-[#ff006e] text-[11px] font-bold">
                        {variant.type}
                      </span>
                      <span className="text-[#2d2d4a] text-[13px]">{variant.form}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {word.examples && word.examples.length > 0 && (
              <div className="mb-2">
                <h3 className="label-warm text-[10px] font-black text-[#4a4a6a] uppercase tracking-[0.12em] mb-0.5 flex items-center gap-1">
                  <BookText className="w-3 h-3 text-[#ff006e]" />
                  {t.examples || '例句'}
                </h3>
                <div className="space-y-1">
                  {word.examples.slice(0, 3).map((ex, i) => (
                    <div key={i} className="border-l-[3px] border-[#ff006e] pl-2.5">
                      <div className="flex items-start gap-1.5">
                        ...
                        {ex.sentence && (
                          <button
                            ...
                            className="p-0.5 text-[#ff006e] hover:text-[#ff006e] hover:bg-[#fff0f5] transition-colors shrink-0"
                          >
                      ...
                      {ex.translation && (
                        <p className="text-[#7a7a9a] text-[11px] leading-snug">{ex.translation}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {word.memory_hint && (
              <div className="mb-2">
                <h3 className="label-warm text-[10px] font-black text-[#4a4a6a] uppercase tracking-[0.12em] mb-0.5 flex items-center gap-1">
                  <Lightbulb className="w-3 h-3 text-[#ff006e]" />
                  {t.memoryHint || '记忆辅助'}
                </h3>
                <p className="text-[13px] text-[#2d2d4a] leading-relaxed bg-[#fff0f5] px-3 py-2 border-[3px] border-[#ff006e]">
                  {word.memory_hint}
                </p>
              </div>
            )}
...
function WordListPanel({ sourceLang, t, onBack, pageSize = 50 }) {
  ...
  const renderPagination = () => {
    ...
    return (
      <div className="flex items-center justify-center gap-1 py-2 border-t-[3px] border-[#1a1a2e] bg-white/40">
        <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
          className={`p-1 transition-colors ${page <= 1 ? 'text-[#7a7a9a] cursor-not-allowed' : 'text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff]'}`}>
          <ChevronLeft className="w-3.5 h-3.5" />
        </button>
        <span className="text-[11px] text-[#7a7a9a] px-2">{page} / {totalPages}</span>
        <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
          className={`p-1 transition-colors ${page >= totalPages ? 'text-[#7a7a9a] cursor-not-allowed' : 'text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff]'}`}>
    ...
    if (loading) {
      return (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-2 text-sm text-[#7a7a9a]">{t.loadingVocab || '加载单词表...'}</span>
        </div>
      )
    }
    ...
    if (filteredWords.length === 0) {
      return (
        <div className="py-16 text-center">
          <BookOpen className="w-8 h-8 text-[#7a7a9a] mx-auto mb-2" />
    ...
    return (
      <div className="flex h-full">
        {allLetterIndex.length > 1 && !searchQuery && (
          <div className="flex flex-col items-center gap-px py-1 border-r-[3px] border-[#1a1a2e] bg-white/50 shrink-0 w-5">
            {allLetterIndex.map(letter => {
              ...
              return (
                <button
                  ...
                  className={`w-4 h-4 flex items-center justify-center text-[8px] font-black transition-colors leading-tight ${
                    onCurrentPage
                      ? 'text-[#2d2d4a] hover:text-[#ff006e] hover:bg-[#fff0f5]'
                      : 'text-[#7a7a9a]/60 hover:text-[#ff006e] hover:bg-[#fff0f5]/50'
                  }`}
                >
                  {letter}
                </button>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto" ref={listRef}>
          {groupedWords.map(([letter, groupWords]) => (
            <div key={letter} id={`letter-${letter}`}>
              <div className="sticky top-0 z-10 px-5 py-1.5 bg-white/90 backdrop-blur-sm border-b-[3px] border-[#1a1a2e]">
                <span className="label-warm text-xs font-bold text-[#7a7a9a] tracking-wider">{letter}</span>
                <span className="text-[10px] text-[#7a7a9a] ml-1.5">{groupWords.length}</span>
              </div>
              {groupWords.map((word) => (
                <div key={word.word} ref={el => { wordRefs.current[word.word] = el }} className="border-b-[3px] border-[#1a1a2e] last:border-b-0">
                  <div
                    ...
                    className="w-full flex items-center gap-3 px-5 py-2.5 hover:bg-[#f0f0ff] transition-colors text-left cursor-pointer"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-black font-display text-[#1a1a2e] ${displayMode === 2 && expandedWord !== word.word ? 'invisible' : ''}`}>{word.word}</span>
                        {word.ipa && (
                          <span className={`text-xs text-[#7a7a9a] ipa-font ${displayMode === 2 && expandedWord !== word.word ? 'invisible' : ''}`}>
                            {word.ipa.startsWith('/') ? word.ipa : `/${word.ipa}/`}
                          </span>
                        )}
                        {word.part_of_speech && (
                          <span className="badge-ochre text-[10px] px-1.5 py-0.5 bg-[#f0f0ff] text-[#4a4a6a] font-bold">
                            {word.part_of_speech}
                          </span>
                        )}
                      </div>
                      {word.meaning && (
                        <p className={`text-xs text-[#4a4a6a] mt-0.5 truncate ${displayMode === 1 && expandedWord !== word.word ? 'invisible' : ''}`}>{meaningOverrides[word.word] || word.meaning}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {expandedWord === word.word && detailLoading[word.word] ? (
                        ...
                        <button
                          ...
                          className="p-1.5 text-[#7a7a9a] hover:text-[#ff006e] hover:bg-[#fff0f5] transition-colors"
  ...
  if (onBack) {
    return (
      <div className="h-full flex flex-col bg-white border-[3px] border-[#1a1a2e] shadow-[2px_2px_0_#1a1a2e] overflow-hidden min-h-0">
        <div className="flex items-center justify-between px-5 py-3.5 border-b-[3px] border-[#1a1a2e] bg-[#fff0f5]">
          <div className="flex items-center gap-2.5">
            <BookOpen className={`w-5 h-5 cursor-pointer transition-colors ${displayMode !== 0 ? 'text-[#ff006e]' : 'text-[#ff006e] hover:text-[#ff006e]'}`} onClick={() => setDisplayMode(v => (v + 1) % 3)} title={displayMode === 0 ? (t.showAll || '显示全部') : displayMode === 1 ? (t.hideMeaning || '隐藏释义') : (t.hideWord || '隐藏单词')} />
            <span className="text-base font-black font-display text-[#1a1a2e]">{t.vocabOverview || '词汇总览'}</span>
            {!loading && words.length > 0 && (
              <span className="badge-ochre text-xs text-[#7a7a9a] bg-[#f0f0ff] px-2 py-0.5">{words.length} {t.wordCount || '词'}</span>
            )}
          </div>
          <button
            onClick={onBack}
            className="btn-ghost flex items-center gap-1.5 px-3 py-1.5 text-sm border-[3px] border-[#1a1a2e] font-black uppercase"
          >
        ...
        <div className="px-4 py-3 border-b-[3px] border-[#1a1a2e]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2 text-sm bg-white border-[3px] border-[#1a1a2e] focus:outline-none focus:shadow-[3px_3px_0_#ff006e] placeholder-[#7a7a9a] transition-all duration-150"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 text-[#7a7a9a] hover:text-[#2d2d4a] transition-colors"
              >
  ...
  return (
    <div className="mt-6">
      <motion.button
        ...
        className="w-full flex items-center justify-between px-5 py-3.5 bg-white backdrop-blur-sm border-[3px] border-[#1a1a2e] hover:border-[#ff006e] transition-all duration-150 shadow-[2px_2px_0_#1a1a2e]"
      >
        <div className="flex items-center gap-2.5">
          <BookOpen className="w-4 h-4 text-[#ff006e]" />
          <span className="text-sm font-black font-display text-[#1a1a2e]">{t.vocabOverview || '词汇总览'}</span>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="overflow-hidden"
          >
            <div className="mt-3 bg-white backdrop-blur-sm border-[3px] border-[#1a1a2e] shadow-[2px_2px_0_#1a1a2e]">
              <div className="p-4 border-b-[3px] border-[#1a1a2e]">
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder={t.searchWordOrMeaning || '搜索单词或释义...'}
                    className="w-full pl-9 pr-9 py-2.5 text-sm bg-white border-[3px] border-[#1a1a2e] focus:outline-none focus:shadow-[3px_3px_0_#ff006e] placeholder-[#7a7a9a] transition-all duration-150"
                  />
                  {searchQuery && (
                    <button
                      onClick={() => setSearchQuery('')}
