...
function LangIcon({ langCode, size = 'md' }) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-md font-bold text-white leading-none border-[2px] border-[var(--border-color)] ${sizeClasses}`}
    ...
    <div ref={containerRef} className="relative">
      {compact ? (
        <button
          ...
          className="flex items-center gap-1.5 text-base font-medium text-[var(--color-dark)] hover:text-[var(--color-dark)] transition-colors"
        >
          <span className="leading-none">
            {isAuto ? <LangIcon langCode="auto" size="md" /> : <LangIcon langCode={value} size="md" />}
          </span>
          <span className="text-[var(--color-dark)]">{currentLabel}</span>
          {nativeLabel && nativeLabel !== currentLabel && (
            <span className="text-xs text-[var(--color-muted)]">[{nativeLabel}]</span>
          )}
          <ChevronDown className={`w-3.5 h-3.5 text-[var(--color-dark)] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
        </button>
      ) : (
        <button
          type="button"
          onClick={() => setOpen(!open)}
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-[var(--radius-sm)] border-[var(--border-width)] transition-all duration-200 text-left group ${
            open
              ? 'border-[var(--color-accent)] bg-[#fde8e8] shadow-[0_0_0_3px_var(--color-accent),3px_3px_0_var(--color-dark)]'
              : 'border-[var(--border-color)] bg-[var(--color-bg)] hover:border-[var(--color-accent)] hover:shadow-[var(--shadow-sm)]'
          }`}
        >
          {isAuto ? (
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isAuto && selectedLang && getSecondary(selectedLang) && (
              <span className="text-xs text-[var(--color-muted)] ml-2">{getSecondary(selectedLang)}</span>
            )}
            {isAuto && (
          ...
          <ChevronDown className={`w-4 h-4 text-[var(--color-dark)] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
        </button>
      )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`absolute z-50 mt-2 bg-[var(--color-card)] rounded-[var(--radius-md)] border-[var(--border-width)] border-[var(--border-color)] shadow-[var(--shadow)] overflow-hidden ${compact ? 'left-0 w-72' : 'w-full'}`}
          >
            <div className="p-3 border-b-[var(--border-width)] border-[var(--border-color)]">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-8 py-2 rounded-lg bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] text-sm text-[var(--color-dark)] placeholder-[var(--color-muted)] focus:outline-none focus:border-[var(--color-accent)] focus:shadow-[0_0_0_3px_var(--color-accent),3px_3px_0_var(--color-dark)] focus:bg-[var(--color-card)] transition-colors"
                />
                {search && (
                  <button type="button" onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-[var(--color-warning)]/20 transition-colors">
            ...
            <div className="max-h-72 overflow-y-auto overscroll-contain">
              {!search && (showAuto || recentLangs.length > 0) && (
                <div className="border-b-[var(--border-width)] border-[var(--border-color)]">
                  {showAuto && (
                    <button
                      type="button"
                      onClick={() => handleSelect('auto')}
                      className={`w-full flex items-center gap-2.5 px-5 py-2 text-sm transition-colors ${
                        isAuto ? 'bg-[#fde8e8] text-[var(--color-accent)]' : 'text-[var(--color-dark)] hover:bg-[var(--color-warning)]/20'
                      }`}
                    >
                  ...
                  {recentLangs.map((lang) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-5 py-1.5 text-sm transition-colors ${
                        value === lang.value ? 'bg-[#fde8e8] text-[var(--color-accent)]' : 'text-[var(--color-dark)] hover:bg-[var(--color-warning)]/20'
                      }`}
                    >
                      <LangIcon langCode={lang.value} size="sm" />
                      <span className={value === lang.value ? 'font-medium' : ''}>{getLabel(lang)}</span>
                      {getSecondary(lang) && <span className="text-xs text-[var(--color-muted)]">{getSecondary(lang)}</span>}
                    </button>
                  ))}
                </div>
              )}
                ...
                return (
                  <div key={family}>
                    <button
                      ...
                      className="w-full flex items-center gap-1.5 px-4 py-2 text-[11px] font-black uppercase text-[var(--color-muted-dark)] tracking-wider hover:bg-[var(--color-warning)]/20 transition-colors"
                    >
                      ...
                      <span className="text-[var(--color-dark)] font-normal normal-case tracking-normal">{langs.length}</span>
                    </button>
      ...
      {MODES.map(({ key, icon: Icon, color }) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => setMode(key)}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium border-[var(--border-width)] transition-all duration-200 ${
              isActive ? 'bg-[var(--color-warning)]/20 text-[var(--color-dark)] border-[var(--border-color)]' : 'text-[var(--color-muted)] hover:text-[var(--color-muted-dark)] hover:bg-[var(--color-bg)] border-transparent'
            }`}
          >
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4">
        ...
        >
          <div className="w-14 h-14 bg-[var(--color-accent)] rounded-[var(--radius-md)] flex items-center justify-center shadow-[var(--shadow-sm)] border-[var(--border-width)] border-[var(--border-color)]">
            <FrogLogo size={32} />
          </div>
          <div>
            <h1 className="text-3xl font-display font-bold text-[var(--color-dark)] leading-tight">
              {t.title || '呱邻国'}
            </h1>
            <p className="text-sm text-[var(--color-muted)]">{t.subtitle || 'Gualingo'}</p>
          </div>
        </motion.div>
        <motion.p
      ...
      <div className="w-full max-w-2xl mx-auto pb-4 px-4">
        <div className="relative bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-md)] shadow-[var(--shadow)] overflow-hidden">
          {/* Mode tabs at top of input */}
          <div className="border-b-[var(--border-width)] border-[var(--border-color)] px-3 pt-2 pb-0">
          ...
          <div className="relative">
            <textarea
              ...
              rows={4}
              className="w-full resize-none bg-transparent border-0 focus:ring-0 focus:outline-none px-4 py-3 text-sm text-[var(--color-dark)] placeholder-[var(--color-muted)] leading-relaxed"
            />
            ...
            <div className="flex items-center justify-end px-3 pb-3">
              <motion.button
                ...
                className={`p-2 rounded-[var(--radius-sm)] border-[var(--border-width)] border-[var(--border-color)] transition-all duration-200 ${
                  loading || !text.trim()
                    ? 'bg-[var(--color-bg)] text-[var(--color-muted)] cursor-not-allowed'
                    : 'bg-[var(--color-accent)] text-white shadow-[var(--shadow)] hover:bg-[var(--color-accent)] hover:shadow-[var(--shadow-lg)]'
                }`}
              >
