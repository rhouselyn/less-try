          ...
          <div className="p-4 border-2 border-dashed border-[#e8a87c]/40 rounded-lg flex flex-wrap gap-2 bg-canvas-50/50 relative">
            <div className="flex flex-wrap gap-2 invisible" aria-hidden="true">
              {data.answer_words.map((_, i) => (
                <span key={`ph-${i}`} className="px-4 py-2 rounded-lg text-sm font-medium">{data.answer_words[i]}</span>
              ))}
            ...
            <div className="absolute inset-0 p-4 flex flex-wrap gap-2 items-start content-start">
              {selectedWords.length === 0 && !answerChecked && (
                <span className="text-stone-300 text-sm pointer-events-none">{t.maskedHint || '点击下方选项填入...'}</span>
              )}
              ...
              <motion.button
                ...
                className="ml-auto p-2 text-warmorange-400 hover:text-warmorange-500 hover:bg-warmorange-50 rounded-lg transition-colors"
