...
function DictionaryStep({ vocab, onToggleSort, sortOrder, progress, processingInfo, sentenceTranslations, selectedSentence, selectedWord, onSentenceClick, onCloseSentenceDetail, onWordClick, onStartLearning, loading, t, currentFileId, sourceLang, preprocessStatus, onBack, fileTitle, onTitleChange, pageSize = 50, dictStateRef, originalText = '' }) {
  ...
  const renderOriginalSentence = (item) => {
    ...
    if (allWords.length === 0) {
      return <div className="font-bold text-[15px] text-[#1a1a2e] mb-1.5 sentence-text">{sentence}</div>
    }
    ...
    return (
      <div className="font-bold text-[15px] text-[#1a1a2e] mb-1.5 leading-relaxed sentence-text">
        {parts.map((part, i) => {
          if (!part) return null
          ...
          if (clickable) {
            return (
              <span
                ...
                className="cursor-pointer px-0.5 -mx-0.5 hover:bg-[#fff0f5] hover:text-amber-800 transition-colors duration-150 border-b border-[#ff006e]/50"
              >
                {part}
              </span>
            )
  ...
  const renderPagination = (currentPage, totalPages, onPageChange) => {
    ...
    return (
      <div className="flex items-center justify-center gap-1 py-1.5 border-t border-[#1a1a2e] bg-white/40">
        <button
          onClick={() => onPageChange(p => Math.max(1, p - 1))}
          disabled={currentPage <= 1}
          className={`p-1 transition-colors ${currentPage <= 1 ? 'text-[#1a1a2e] cursor-not-allowed' : 'text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff]'}`}
        >
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).filter(p => {
          ...
          p === '...' ? (
            <span key={`dots-${i}`} className="text-[10px] text-[#1a1a2e] px-0.5">...</span>
          ) : (
            <button
              key={p}
              onClick={() => onPageChange(p)}
              className={`min-w-[22px] h-[22px] flex items-center justify-center text-[10px] transition-colors ${
                currentPage === p
                  ? 'bg-[#fff0f5] text-amber-700 font-black uppercase'
        ...
        <button
          ...
          className={`p-1 transition-colors ${currentPage >= totalPages ? 'text-[#1a1a2e] cursor-not-allowed' : 'text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff]'}`}
        >
    ...
    >
      <div className="flex items-center gap-3 px-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-black uppercase text-[#1a1a2e]">
        ...
        {fileTitle && !editingTitle && (
          ...
          >
            <span className="truncate text-base font-bold text-[#2d2d4a] group-hover:text-[#1a1a2e] transition-colors">{fileTitle}</span>
            <Pencil className="w-2.5 h-2.5 text-[#1a1a2e] group-hover:text-[#7a7a9a] shrink-0 transition-colors" />
          </button>
        )}
        ...
        {editingTitle && (
          <input
            ...
            className="text-[13px] font-bold text-[#2d2d4a] bg-transparent border-b border-[3px] border-[#1a1a2e] px-1 py-0.5 max-w-[250px] focus:outline-none focus:border-[#ff006e] transition-colors"
          />
        )}
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            {preprocessStatus ? (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] text-blue-500 font-bold truncate">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[#7a7a9a] tabular-nums whitespace-nowrap">
                ...
                <span className="text-[10px] text-[#7a7a9a] whitespace-nowrap">
                  {t.processingSentences || '处理句子中...'}
                </span>
              </div>
            ) : wordGenProgress && wordGenProgress.completed < wordGenProgress.total ? (
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[#ff006e] tabular-nums whitespace-nowrap">
                ...
                <span className="text-[10px] text-[#ff006e] whitespace-nowrap">
      ...
      <div className="flex gap-6 flex-1 min-h-0" style={{ overflow: 'hidden' }}>
        <div className="w-1/2 flex flex-col min-h-0" style={{ overflow: 'hidden' }}>
          <div className="bg-white border-[3px] border-[#1a1a2e] shadow-[3px_3px_0_#1a1a2e] overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="px-5 py-3.5 border-b border-[#1a1a2e] bg-white/60">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 shrink-0" style={{ minWidth: '140px' }}>
                  <Languages className={`w-4 h-4 transition-colors cursor-pointer ${sentenceDisplayMode !== 0 ? 'text-[#ff006e]' : 'text-[#4a4a6a] hover:text-[#ff006e]'}`} onClick={(e) => { e.stopPropagation(); setSentenceDisplayMode(v => (v + 1) % 3) }} title={sentenceDisplayMode === 0 ? t.showAll : sentenceDisplayMode === 1 ? t.hideTranslation : t.hideOriginal} />
                  <h3 className="text-sm font-black uppercase text-[#1a1a2e] font-display">
                    <span className="cursor-pointer select-none" onClick={handleToggleShowOriginal}>
                      ...
                      <span className="text-[#1a1a2e] mx-1.5">/</span>
                ...
                <div className="relative w-1/2 ml-auto">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#1a1a2e]" />
                  <input
                    type="text"
            ...
            <div className="flex-1 overflow-y-scroll min-h-0" ref={sentenceListRef} style={{ scrollbarGutter: 'stable' }}>
              {showOriginal ? (
                <div className="p-4">
                  <pre className="text-sm text-[#1a1a2e] leading-relaxed whitespace-pre-wrap font-sans">{originalText || safeSentenceTranslations.map(item => item.sentence || '').join('\n')}</pre>
                </div>
                      ...
                      <div key={originalIndex} ref={el => { sentenceRefs.current[originalIndex] = el }}>
                        <motion.div
                          ...
                          className={`p-4 cursor-pointer transition-colors ${
                            selectedSentence === originalIndex ? 'bg-[#fff0f5]/60' : 'hover:bg-[#fff0f5]/30'
                          }`}
                          onClick={() => {
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <Volume2 className="w-3.5 h-3.5 text-[#1a1a2e] hover:text-[#ff006e] shrink-0 mt-1 transition-colors" onClick={(e) => speakWord(item.sentence || '', e)} />
                          </div>
                        </motion.div>
                        <AnimatePresence>
                          {selectedSentence === originalIndex && (
                            ...
                            >
                              <div className="border-t border-[#1a1a2e] p-4 bg-white/50">
                                <SentenceDetail
                ...
                <div className="py-16 text-center">
                  <Languages className="w-10 h-10 mx-auto mb-3 text-[#1a1a2e]" />
                  <p className="text-[#7a7a9a] text-sm">{sentenceSearch ? (t.noMatchingSentences || '没有找到匹配的句子') : t.loading}</p>
                </div>
              )}
            </div>
        ...
        <div className="w-1/2 flex flex-col min-h-0" style={{ overflow: 'hidden' }}>
          <div className="bg-white border-[3px] border-[#1a1a2e] shadow-[3px_3px_0_#1a1a2e] overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="px-5 py-3.5 border-b border-[#1a1a2e] bg-white/60">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 shrink-0" style={{ minWidth: '140px' }}>
                  <BookOpen className={`w-4 h-4 transition-colors cursor-pointer ${vocabDisplayMode !== 0 ? 'text-[#ff006e]' : 'text-[#4a4a6a] hover:text-[#ff006e]'}`} onClick={(e) => { e.stopPropagation(); setVocabDisplayMode(v => (v + 1) % 3) }} title={vocabDisplayMode === 0 ? t.showAll : vocabDisplayMode === 1 ? t.hideMeaning : t.hideWord} />
                  <h3 className="text-sm font-black uppercase text-[#1a1a2e] font-display">
            ...
            <div className="flex-1 flex min-h-0">
              {((!showGlobalVocab && allLetterIndex.length > 1) || (showGlobalVocab && allGlobalLetterIndex.length > 1)) && (
                <div className="flex flex-col items-center gap-px py-1 border-r border-[#1a1a2e] bg-white/40 w-5 shrink-0 overflow-y-auto">
                    ...
                    return (
                      <button
                        ...
                        className={`w-4 h-4 flex items-center justify-center text-[8px] font-black uppercase transition-colors shrink-0 ${
                          onCurrentPage
                            ? 'text-[#2d2d4a] hover:text-[#ff006e] hover:bg-[#fff0f5]'
                            : 'text-[#1a1a2e]/60 hover:text-[#ff006e] hover:bg-[#fff0f5]/50'
                        }`}
                      >
                        {letter}
                      </button>
              ...
              {showGlobalVocab ? (
                globalVocabLoading ? (
                  ...
                  <div className="py-16 text-center">
                    <BookOpen className="w-10 h-10 mx-auto mb-3 text-[#1a1a2e]" />
                    <p className="text-[#7a7a9a] text-sm">{vocabSearch ? (t.noMatchFound || '没有找到匹配的单词') : (t.noWordsYetHint || '暂无单词')}</p>
                  </div>
                ) : (
                <div className="space-y-3">
                  {groupedGlobalVocab.map(([letter, words], groupIdx) => (
                    <div key={letter} id={`dict-group-${letter}`}>
                      ...
                      >
                        <span className="text-xs font-bold text-[#ff006e]/80 tracking-widest">{letter}</span>
                      </motion.div>
                          ...
                          return (
                            ...
                            >
                              <button
                                ...
                                className="w-full text-left px-4 py-2.5 flex items-center gap-2 hover:bg-[#fff0f5]/40 transition-colors group"
                              >
                                <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap select-text">
                                  <span className={`text-[14px] font-black uppercase text-[#1a1a2e] tracking-tight shrink-0 ${vocabDisplayMode === 2 && !isExpanded ? 'invisible' : ''}`}>
                                    {word.word}
                                  </span>
                                  {word.ipa && (
                                    <span className={`text-[11px] text-[#7a7a9a] ipa-font shrink-0 ${vocabDisplayMode === 2 && !isExpanded ? 'invisible' : ''}`}>
                                  ...
                                  <span className={`text-[12px] text-[#4a4a6a] truncate ${vocabDisplayMode === 1 && !isExpanded ? 'invisible' : ''}`}>
                                    {meaningOverrides[wordKey] || word.meaning}
                                  </span>
                                </div>
                                {isExpanded && (
                                  <RefreshCw
                                    className="w-3.5 h-3.5 text-[#1a1a2e] hover:text-[#ff006e] shrink-0 transition-colors"
                              ...
                              <AnimatePresence>
                                {isExpanded && (
                                  <motion.div
                                    ...
                                    className="overflow-hidden"
                                  >
                                    <div className="px-4 pb-3.5 border-t border-[#1a1a2e]">
                                      {isLoading ? (
                                        <div className="pt-4 flex flex-col items-center justify-center gap-3">
                                          <Loader2 className="w-5 h-5 animate-spin text-[#ff006e]" />
                                          <p className="text-[12px] text-[#7a7a9a]">{t.generatingWordDetails || '正在生成单词详解...'}</p>
                                        </div>
                                      ) : detail ? (
                                        <div className="pt-3">
                                          <div className="mb-2">
                                            <h3 className="label-warm mb-0.5 flex items-center gap-1">
                                              <Brain className="w-3 h-3 text-[#ff006e]" />
                                              {t.definition || '释义'}
                                            </h3>
                                        ...
                                        <div className="pt-3 text-center text-[#7a7a9a] text-[12px]">
                                          {t.noDetails || '暂无详情'}
                                        </div>
                                      )}
                                    </div>
              ...
              {groupedVocab.length === 0 ? (
                <div className="py-16 text-center">
                  <BookOpen className="w-10 h-10 mx-auto mb-3 text-[#1a1a2e]" />
                  <p className="text-[#7a7a9a] text-sm">{loading ? t.loading : (vocabSearch ? (t.noMatchFound || '没有找到匹配的单词') : t.loading)}</p>
                </div>
              ) : (
              <div className="space-y-3">
