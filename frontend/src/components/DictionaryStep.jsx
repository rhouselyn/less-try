...
function DictionaryStep({ vocab, onToggleSort, sortOrder, progress, processingInfo, sentenceTranslations, selectedSentence, selectedWord, onSentenceClick, onCloseSentenceDetail, onWordClick, onStartLearning, loading, t, currentFileId, sourceLang, preprocessStatus, onBack, fileTitle, onTitleChange, pageSize = 50, dictStateRef, originalText = '' }) {
  ...
  const renderOriginalSentence = (item) => {
    ...
    if (allWords.length === 0) {
      return <div className="font-medium text-[15px] text-[var(--color-dark)] mb-1.5 sentence-text">{sentence}</div>
    }
    ...
    return (
      <div className="font-medium text-[15px] text-[var(--color-dark)] mb-1.5 leading-relaxed sentence-text">
        {parts.map((part, i) => {
          if (!part) return null
          ...
          if (clickable) {
            return (
              <span
                ...
                className="cursor-pointer rounded px-0.5 -mx-0.5 hover:bg-[var(--color-warning)]/20 hover:text-[var(--color-dark)] transition-colors duration-150 border-b border-[var(--color-accent)]/50"
              >
                {part}
              </span>
            )
  ...
  const renderTranslation = (item) => {
    ...
    if (!text) return null
    return <div className={`text-[var(--color-muted-dark)] text-[14px] ${sentenceDisplayMode === 1 ? 'invisible' : ''}`}>{text}</div>
  }

  const renderPagination = (currentPage, totalPages, onPageChange) => {
    if (totalPages <= 1) return null
    return (
      <div className="flex items-center justify-center gap-1 py-1.5 border-t border-[var(--border-color)]/60 bg-[var(--color-bg)]/40">
        <button
          onClick={() => onPageChange(p => Math.max(1, p - 1))}
          disabled={currentPage <= 1}
          className={`p-1 rounded transition-colors ${currentPage <= 1 ? 'text-[var(--color-dark)]/30 cursor-not-allowed' : 'text-[var(--color-muted)] hover:text-[var(--color-muted-dark)] hover:bg-[var(--color-bg)]'}`}
        >
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).filter(p => {
          ...
          p === '...' ? (
            <span key={`dots-${i}`} className="text-[10px] text-[var(--color-dark)]/40 px-0.5">...</span>
          ) : (
            <button
              key={p}
              ...
              className={`min-w-[22px] h-[22px] flex items-center justify-center text-[10px] rounded transition-colors ${
                currentPage === p
                  ? 'bg-[var(--color-accent)]/20 text-[var(--color-dark)] font-black'
                  : 'text-[var(--color-muted)] hover:text-[var(--color-muted-dark)] hover:bg-[var(--color-bg)]'
              }`}
            >
              {p}
            </button>
        ...
        <button
          ...
          className={`p-1 rounded transition-colors ${currentPage >= totalPages ? 'text-[var(--color-dark)]/30 cursor-not-allowed' : 'text-[var(--color-muted)] hover:text-[var(--color-muted-dark)] hover:bg-[var(--color-bg)]'}`}
        >
    ...
    >
      <div className="flex items-center gap-3 px-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-black uppercase text-[var(--color-dark)]">
        ...
        {fileTitle && !editingTitle && (
          ...
          >
            <span className="truncate text-base font-medium text-[var(--color-muted-dark)] group-hover:text-[var(--color-dark)] transition-colors">{fileTitle}</span>
            <Pencil className="w-2.5 h-2.5 text-[var(--color-dark)]/40 group-hover:text-[var(--color-muted)] shrink-0 transition-colors" />
          </button>
        )}
        ...
        {editingTitle && (
          <input
            ...
            className="text-[13px] font-medium text-[var(--color-muted-dark)] bg-transparent border-b border-[var(--border-color)] px-1 py-0.5 max-w-[250px] focus:outline-none focus:border-[var(--color-accent)] transition-colors"
          />
        )}
        ...
        {(preprocessStatus || (currentFileId && ((processingInfo && safeProcessingInfo.total > 0 && progress < 100) || (wordGenProgress && wordGenProgress.completed < wordGenProgress.total)))) && (
          <div className="flex items-center gap-2.5 shrink-0">
            {preprocessStatus ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[var(--color-accent)] tabular-nums whitespace-nowrap">
      ...
      <div className="flex gap-6 flex-1 min-h-0" style={{ overflow: 'hidden' }}>
        <div className="w-1/2 flex flex-col min-h-0" style={{ overflow: 'hidden' }}>
          <div className="bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="px-5 py-3.5 border-b-[2px] border-[var(--border-color)] bg-[var(--color-card)]/60">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 shrink-0" style={{ minWidth: '140px' }}>
                  <Languages className={`w-4 h-4 transition-colors cursor-pointer ${sentenceDisplayMode !== 0 ? 'text-[var(--color-accent)]' : 'text-[var(--color-muted-dark)] hover:text-[var(--color-accent)]'}`} onClick={(e) => { e.stopPropagation(); setSentenceDisplayMode(v => (v + 1) % 3) }} title={sentenceDisplayMode === 0 ? t.showAll : sentenceDisplayMode === 1 ? t.hideTranslation : t.hideOriginal} />
                  <h3 className="text-sm font-black uppercase text-[var(--color-dark)] font-display">
                    <span className="cursor-pointer select-none" onClick={handleToggleShowOriginal}>
                      ...
                      <span className="text-[var(--color-dark)]/40 mx-1.5">/</span>
                ...
                <div className="relative w-1/2 ml-auto">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--color-dark)]/40" />
                  <input
                    type="text"
            ...
            <div className="flex-1 overflow-y-scroll min-h-0" ref={sentenceListRef} style={{ scrollbarGutter: 'stable' }}>
              {showOriginal ? (
                <div className="p-4">
                  <pre className="text-sm text-[var(--color-dark)] leading-relaxed whitespace-pre-wrap font-sans">{originalText || safeSentenceTranslations.map(item => item.sentence || '').join('\n')}</pre>
                </div>
                      ...
                      <div key={originalIndex} ref={el => { sentenceRefs.current[originalIndex] = el }}>
                        <motion.div
                          ...
                          className={`p-4 cursor-pointer transition-colors ${
                            selectedSentence === originalIndex ? 'bg-[var(--color-accent)]/10' : 'hover:bg-[var(--color-accent)]/5'
                          }`}
                          onClick={() => {
                        ...
                        >
                          <div className="flex items-start gap-2">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className={`text-[var(--color-muted-dark)] text-[14px] sentence-text ${sentenceDisplayMode === 1 && selectedSentence !== originalIndex ? 'invisible' : ''}`}>
                                {item.translation_result?.tokenized_translation || ''}
                              </div>
                            </div>
                            <Volume2 className="w-3.5 h-3.5 text-[var(--color-dark)]/40 hover:text-[var(--color-accent)] shrink-0 mt-1 transition-colors" onClick={(e) => speakWord(item.sentence || '', e)} />
                          </div>
                        </motion.div>
                        <AnimatePresence>
                          {selectedSentence === originalIndex && (
                            ...
                            >
                              <div className="border-t border-[var(--border-color)]/60 p-4 bg-[var(--color-card)]/50">
                                <SentenceDetail
                ...
                <div className="py-16 text-center">
                  <Languages className="w-10 h-10 mx-auto mb-3 text-[var(--color-dark)]/30" />
                  <p className="text-[var(--color-muted)] text-sm">{sentenceSearch ? (t.noMatchingSentences || '没有找到匹配的句子') : t.loading}</p>
                </div>
              )}
            </div>
        ...
        <div className="w-1/2 flex flex-col min-h-0" style={{ overflow: 'hidden' }}>
          <div className="bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="px-5 py-3.5 border-b-[2px] border-[var(--border-color)] bg-[var(--color-card)]/60">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 shrink-0" style={{ minWidth: '140px' }}>
                  <BookOpen className={`w-4 h-4 transition-colors cursor-pointer ${vocabDisplayMode !== 0 ? 'text-[var(--color-accent)]' : 'text-[var(--color-muted-dark)] hover:text-[var(--color-accent)]'}`} onClick={(e) => { e.stopPropagation(); setVocabDisplayMode(v => (v + 1) % 3) }} title={vocabDisplayMode === 0 ? t.showAll : vocabDisplayMode === 1 ? t.hideMeaning : t.hideWord} />
                  <h3 className="text-sm font-black uppercase text-[var(--color-dark)] font-display">
            ...
            <div className="flex-1 flex min-h-0">
              {((!showGlobalVocab && allLetterIndex.length > 1) || (showGlobalVocab && allGlobalLetterIndex.length > 1)) && (
                <div className="flex flex-col items-center gap-px py-1 border-r border-[var(--border-color)]/60 bg-[var(--color-bg)]/40 w-5 shrink-0 overflow-y-auto">
                    ...
                    return (
                      <button
                        ...
                        className={`w-4 h-4 flex items-center justify-center text-[8px] font-black uppercase rounded transition-colors shrink-0 ${
                          onCurrentPage
                            ? 'text-[var(--color-muted-dark)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10'
                            : 'text-[var(--color-dark)]/25 hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10'
                        }`}
                      >
                        {letter}
                      </button>
              ...
              {showGlobalVocab ? (
                globalVocabLoading ? (
                  <div className="py-16 text-center">
                    <Loader2 className="w-8 h-8 mx-auto mb-3 animate-spin text-[var(--color-accent)]" />
                    <p className="text-[var(--color-muted)] text-sm">{t.loading}</p>
                  </div>
                ) : groupedGlobalVocab.length === 0 ? (
                  <div className="py-16 text-center">
                    <BookOpen className="w-10 h-10 mx-auto mb-3 text-[var(--color-dark)]/30" />
                    <p className="text-[var(--color-muted)] text-sm">{vocabSearch ? (t.noMatchFound || '没有找到匹配的单词') : (t.noWordsYetHint || '暂无单词')}</p>
                  </div>
                ) : (
                  ...
                  {groupedGlobalVocab.map(([letter, words], groupIdx) => (
                    <div key={letter} id={`dict-group-${letter}`}>
                      <motion.div
                        ...
                        className="sticky top-0 z-10 backdrop-blur-sm bg-[var(--color-card)]/80 px-4 py-1.5 border-b border-[var(--border-color)]/40 mb-1"
                      >
                        <span className="text-xs font-bold text-[var(--color-accent)]/80 tracking-widest">{letter}</span>
                      </motion.div>
                          ...
                          return (
                            ...
                            >
                              <button
                                ...
                                className="w-full text-left px-4 py-2.5 flex items-center gap-2 hover:bg-[var(--color-accent)]/10 transition-colors group"
                              >
                                <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap select-text">
                                  <span className={`text-[14px] font-black text-[var(--color-dark)] tracking-tight shrink-0 ${vocabDisplayMode === 2 && !isExpanded ? 'invisible' : ''}`}>
                                    {word.word}
                                  </span>
                                  {word.ipa && (
                                    <span className={`text-[11px] text-[var(--color-muted)] ipa-font shrink-0 ${vocabDisplayMode === 2 && !isExpanded ? 'invisible' : ''}`}>
                                      {word.ipa.startsWith('/') ? word.ipa : `/${word.ipa}/`}
                                    </span>
                                  )}
                                  {word.part_of_speech && (
                                    <span className="text-[10px] px-1.5 py-0.5 bg-[var(--color-bg)] text-[var(--color-muted-dark)] rounded font-medium tracking-wide shrink-0">
                                      {word.part_of_speech}
                                    </span>
                                  )}
                                  <span className={`text-[12px] text-[var(--color-muted-dark)] truncate ${vocabDisplayMode === 1 && !isExpanded ? 'invisible' : ''}`}>
                                    {meaningOverrides[wordKey] || word.meaning}
                                  </span>
                                </div>
                                {isExpanded && (
                                  <RefreshCw
                                    className="w-3.5 h-3.5 text-[var(--color-dark)]/40 hover:text-[var(--color-accent)] shrink-0 transition-colors"
                              ...
                              <AnimatePresence>
                                {isExpanded && (
                                  <motion.div
                                    ...
                                    className="overflow-hidden"
                                  >
                                    <div className="px-4 pb-3.5 border-t border-[var(--border-color)]/40">
                                      {isLoading ? (
                                        <div className="pt-4 flex flex-col items-center justify-center gap-3">
                                          <Loader2 className="w-5 h-5 animate-spin text-[var(--color-accent)]" />
                                          <p className="text-[12px] text-[var(--color-muted)]">{t.generatingWordDetails || '正在生成单词详解...'}</p>
                                        </div>
                                      ) : detail ? (
                                        <div className="pt-3">
                                          <div className="mb-2">
                                        ...
                                        <div className="pt-3 text-center text-[var(--color-muted)] text-[12px]">
                                          {t.noDetails || '暂无详情'}
                                        </div>
                                      )}
                                    </div>
              ...
              {groupedVocab.length === 0 ? (
                <div className="py-16 text-center">
                  <BookOpen className="w-10 h-10 mx-auto mb-3 text-[var(--color-dark)]/30" />
                  <p className="text-[var(--color-muted)] text-sm">{loading ? t.loading : (vocabSearch ? (t.noMatchFound || '没有找到匹配的单词') : t.loading)}</p>
                </div>
              ) : (
              <div className="space-y-3">
