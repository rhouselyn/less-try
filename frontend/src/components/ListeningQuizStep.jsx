    ...
    >
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          {totalItemsInUnit > 0 && (
            <span className="text-sm text-[var(--color-muted-dark)] font-medium">{t.step || '第'} {stepInUnit} / {totalItemsInUnit} {t.question || '题'}</span>
          )}
          {!isChecked && !isSkipped && (
            <motion.button
              onClick={handleSkipListening}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[var(--color-muted)] hover:text-[var(--color-accent)] hover:bg-[#fde8e8] rounded-lg transition-colors"
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-3">
            <motion.button
              ...
              className="p-3 text-[var(--color-accent)] hover:text-[var(--color-accent)] hover:bg-[#fde8e8] rounded-full transition-colors"
            >
            ...
            <motion.button
              ...
              className="p-3 text-[var(--color-muted)] hover:text-[var(--color-accent)] hover:bg-[#fde8e8] rounded-full transition-colors"
          ...
          <div className="p-4 border-[var(--border-width)] border-dashed border-[var(--border-color)] rounded-[var(--radius-sm)] flex flex-wrap gap-2 bg-[var(--color-bg)]/50 relative">
            ...
            <div className="absolute inset-0 p-4 flex flex-wrap gap-2 items-center">
              {selectedWords.length === 0 && (
                <span className="italic text-[var(--color-muted)] text-sm pointer-events-none">{t.tapToBuildSentence || '按顺序点击下方单词组成句子'}</span>
              )}
