  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 border-[3px] border-[#1a1a2e] bg-white hover:bg-[#f0f0ff] transition-colors text-sm"
      >
        <LangIcon langCode={value} size="sm" />
        <span className="text-[#1a1a2e] flex-1 text-left">{selectedLang?.native || value}</span>
        <ChevronDown className={`w-3.5 h-3.5 text-[#7a7a9a] transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-white border-[3px] border-[#1a1a2e] shadow-[4px_4px_0_#1a1a2e] overflow-hidden">
          <div className="p-2 border-b-[3px] border-[#1a1a2e]">
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search..."
              className="w-full px-2 py-1.5 bg-white border-[3px] border-[#1a1a2e] text-xs text-[#1a1a2e] placeholder-[#7a7a9a] focus:outline-none focus:shadow-[3px_3px_0_#ff006e]"
              autoFocus
            />
          </div>
          <div className="max-h-48 overflow-y-auto">
            {recentFiltered.length > 0 && (
              <>
                <div className="px-3 py-1 text-[10px] text-[#7a7a9a] font-black uppercase">Recent</div>
                {recentFiltered.map(l => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs transition-colors ${
                      value === l.value ? 'bg-[#fff0f5] text-[#cc0058]' : 'text-[#2d2d4a] hover:bg-white'
                    }`}
                  >
            ...
            {commonFiltered.length > 0 && (
              <>
                <div className="px-3 py-1 text-[10px] text-[#7a7a9a] font-black uppercase">Common</div>
            ...
            {otherFiltered.length > 0 && (
              <>
                <div className="px-3 py-1 text-[10px] text-[#7a7a9a] font-black uppercase border-t-[3px] border-[#1a1a2e]">All Languages</div>
                {otherFiltered.map(l => (
                  <button
                    key={l.value}
                    type="button"
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex items-start justify-end p-6 bg-[#1a1a2e]/50 backdrop-blur-sm"
        onClick={onClose}
      >
        <motion.div
          ...
          className="bg-white border-[3px] border-[#1a1a2e] shadow-[4px_4px_0_#1a1a2e] w-[340px] overflow-hidden mt-2"
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-5 py-4 border-b-[3px] border-[#1a1a2e]">
            <div className="flex items-center gap-2.5">
              <Settings className="w-4 h-4 text-[#4a4a6a]" />
              <h2 className="font-display text-xs font-black uppercase text-[#1a1a2e]">{t.settings || '设置'}</h2>
            </div>
            <button
              onClick={onClose}
              className="btn-ghost p-1 text-[#7a7a9a] hover:text-[#2d2d4a] border-[3px] border-[#1a1a2e] hover:bg-[#ff006e] hover:text-white transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          {loading ? (
            <div className="p-8 flex justify-center">
              <Loader2 className="w-5 h-5 animate-spin text-[#7a7a9a]" />
            </div>
          ) : (
            <div className="p-5 space-y-4 overflow-y-auto max-h-[70vh]">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="label-warm text-[10px] font-black text-[#7a7a9a] uppercase tracking-widest">
                    {t.apiConfig || 'API 配置'} {currentIndex + 1}/{configs.length}
                  </span>
                  {configs.length > 1 && (
                    <button
                      onClick={() => removeConfig(currentIndex)}
                      className="flex items-center gap-1 text-[10px] text-[#7a7a9a] hover:text-[#ef476f] transition-colors"
                    >
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={`flex-shrink-0 w-7 h-7 flex items-center justify-center transition-all ${
                        isFirst
                          ? 'text-[#7a7a9a] cursor-not-allowed'
                          : 'text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none'
                      }`}
                    >
                      <ChevronLeft className="w-4 h-4" />
                    </button>

                    <div className="flex-1 min-w-0 overflow-hidden border-[3px] border-[#1a1a2e] bg-white">
                      <AnimatePresence mode="wait" custom={direction}>
                        ...
                        >
                          <div>
                            <label className="label-warm flex items-center gap-1.5 text-[10px] font-black text-[#7a7a9a] uppercase tracking-widest mb-1.5">
                              <Key className="w-3 h-3" />
                              API Key
                              {current?.has_key && <span className="text-[10px] text-[#06d6a0] normal-case tracking-normal">● {t.configured || '已配置'}</span>}
                            </label>
                            <input
                              type="password"
                              ...
                              className="w-full px-3 py-2 text-xs bg-white border-[3px] border-[#1a1a2e] focus:outline-none focus:shadow-[3px_3px_0_#ff006e] transition-all duration-150 placeholder:text-[#7a7a9a]"
                            />
                            {current?.has_key && !current?.api_key && (
                              <p className="text-[11px] text-[#7a7a9a] mt-1">{t.leaveEmptyKeepKey || '留空则保持当前 Key 不变'}</p>
                            )}
                          </div>
                    ...
                    {isLast ? (
                      <button
                        onClick={addConfig}
                        className="flex-shrink-0 w-7 h-7 flex items-center justify-center text-[#ff006e] hover:text-[#ff006e] hover:bg-[#fff0f5] transition-all active:translate-x-[3px] active:translate-y-[3px] active:shadow-none"
                      >
                      ...
                      <button
                        onClick={goNext}
                        className="flex-shrink-0 w-7 h-7 flex items-center justify-center text-[#7a7a9a] hover:text-[#2d2d4a] hover:bg-[#f0f0ff] transition-all active:translate-x-[3px] active:translate-y-[3px] active:shadow-none"
                      >
                      ...
                      {configs.map((_, i) => (
                        <button
                          ...
                          className={`transition-all duration-150 ${
                            i === currentIndex
                              ? 'w-4 h-1.5 bg-[#ff006e]'
                              : 'w-1.5 h-1.5 bg-[#7a7a9a] hover:bg-[#1a1a2e]'
                          }`}
                        />
                      ))}
                    </div>
              ...
              <div className="pt-1">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#7a7a9a]">{t.retryIntervalDesc || '每次API请求之间的等待时间'}</span>
                    <span className="text-[11px] font-black text-[#ff006e]">{retryInterval.toFixed(1)}s</span>
                  </div>
                  <div className="relative">
                    <input
                      type="range"
                      ...
                      className="w-full h-2 appearance-none cursor-pointer bg-[#f0f0ff]"
                      style={{
                        background: `linear-gradient(to right, #ff006e 0%, #ff006e ${((retryInterval - 0.1) / (20 - 0.1)) * 100}%, #f0f0ff ${((retryInterval - 0.1) / (20 - 0.1)) * 100}%, #f0f0ff 100%)`
                      }}
                    />
                    <div className="flex justify-between mt-1">
                      <span className="text-[10px] text-[#7a7a9a]">0.1s</span>
                      <span className="text-[10px] text-[#7a7a9a]">20s</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#7a7a9a]">{t.wordsPerPage || '每页显示单词数'}</span>
                    <span className="text-[11px] font-black text-[#ff006e]">{localPageSize}</span>
                  </div>
                  <div className="relative">
                    <input
                      type="range"
                      ...
                      className="w-full h-2 appearance-none cursor-pointer bg-[#f0f0ff]"
                      style={{
                        background: `linear-gradient(to right, #ff006e 0%, #ff006e ${((localPageSize - 10) / (200 - 10)) * 100}%, #f0f0ff ${((localPageSize - 10) / (200 - 10)) * 100}%, #f0f0ff 100%)`
                      }}
                    />
                    <div className="flex justify-between mt-1">
                      <span className="text-[10px] text-[#7a7a9a]">10</span>
                      <span className="text-[10px] text-[#7a7a9a]">200</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <motion.button
                onClick={handleSave}
                disabled={saving}
                className="btn-primary w-full py-2.5 bg-[#1a1a2e] text-white text-xs font-bold border-[3px] border-[#1a1a2e] hover:bg-[#2d2d4a] disabled:bg-[#7a7a9a] disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
              >
                {saving ? (
