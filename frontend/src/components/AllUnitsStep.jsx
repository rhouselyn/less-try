  ...
  const renderUnitCard = (unit, index, onClick, keyPrefix, isUnlocked, phaseNumber) => {
    ...
    return (
      <motion.button
        ...
        disabled={isLocked}
        className={`relative flex flex-col items-center justify-center rounded-[var(--radius-sm)] border-[var(--border-width)] border-[var(--border-color)] transition-all duration-200 ${
          isCompleted
            ? 'bg-[#e6f5f3] border-[var(--color-success)]'
            : isLocked
            ? 'bg-[var(--color-bg)] cursor-not-allowed'
            : isCurrent
            ? 'bg-[var(--color-highlight)] border-[var(--color-accent)] shadow-[var(--shadow-sm)]'
            : 'bg-[var(--color-highlight)] hover:border-[var(--color-accent)]'
        }`}
      ...
      >
        {isLocked ? (
          ...
          <>
            <span className="text-base font-black uppercase text-[var(--color-success)]">{index + 1}</span>
                  ...
                  <Star
                    ...
                    className={`w-3 h-3 ${
                      i < starCount
                        ...
                        : 'text-[var(--color-muted)] fill-[var(--color-muted)]'
                    }`}
                  />
                ))}
              </div>
          ...
          <>
            <span className={`text-base font-black uppercase text-[var(--color-accent)]`}>{index + 1}</span>
            {isGenerating ? (
              <Loader2 className="w-3 h-3 animate-spin text-[var(--color-accent)] mt-0.5" />
            ) : (
              <div className={`w-4 h-[2px] rounded-full mt-0.5 ${isCurrent ? 'bg-[var(--color-accent)]' : 'bg-[var(--color-accent)]/60'}`} />
            )}
          </>
        )}
      </motion.button>
    ...
    if (!units || units.length === 0 || (phaseNumber === 2 && units.length === 1 && units[0]?.no_eligible_sentences)) {
      return (
        <div className="py-12 text-center">
          <p className="text-xs text-[var(--color-muted)]">{t.noPracticeContent || '暂无可练习内容'}</p>
        </div>
      );
    }
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-medium text-[var(--color-muted)] tabular-nums">{completed}<span className="text-[var(--color-muted)]">/{total}</span></span>
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1 text-[var(--color-muted)] hover:text-[var(--color-dark)] disabled:text-[var(--color-muted)] disabled:cursor-not-allowed transition-colors"
              >
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <span className="text-[11px] text-[var(--color-muted)] tabular-nums min-w-[60px] text-center">
        ...
        <div className="w-full h-1.5 bg-[var(--color-bg)] rounded-full overflow-hidden mb-5">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${progress}%` }}
            transition={{ duration: 0.6, ease: 'easeOut' }}
            className={`h-full rounded-full ${
              phaseNumber === 1 ? 'bg-[var(--color-success)]' : 'bg-[#457b9d]'
            }`}
          />
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-6">
        ...
        {fileTitle && (
          <span className="text-base font-medium text-[var(--color-muted-dark)] truncate max-w-[240px]">
            {fileTitle}
          </span>
        )}
        ...
        <label className="flex items-center gap-1.5 cursor-pointer select-none group mr-1">
          <span className="text-xs text-[var(--color-muted)] group-hover:text-[var(--color-dark)] transition-colors flex items-center gap-1">
          ...
          <div className="relative inline-flex items-center">
            ...
            />
            <div className="toggle-warm-off peer-checked:bg-[var(--color-accent)] peer-focus:outline-none" />
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 text-center">
          <h2 className="text-xl font-black uppercase font-display text-[var(--color-dark)]">
            {t.learningUnits || '学习单元'}
          </h2>
          <p className="text-xs text-[var(--color-muted)] mt-1">{t.completeUnitsInOrder || '按顺序完成单元，解锁下一单元'}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-[var(--color-card)] rounded-[var(--radius-md)] shadow-[var(--shadow)] border-[var(--border-width)] border-[var(--border-color)] overflow-hidden">
            <div className="bg-[var(--color-bg)] border-b-[var(--border-width)] border-b-[var(--border-color)] px-3 pt-2.5">
              <div className="flex gap-1 relative">
                <motion.div
                  className="absolute top-0 bottom-0 bg-[var(--color-card)] rounded-t-xl shadow-[var(--shadow-sm)]"
                  ...
                  return (
                    ...
                    >
                      <div className={`flex items-center gap-1.5 transition-colors duration-300 ${
                        isActive ? 'text-[var(--color-dark)]' : 'text-[var(--color-muted)] hover:text-[var(--color-muted-dark)]'
                      }`}>
                        <Icon className="w-3.5 h-3.5" />
                        <span>{tab.label}</span>
                        {tab.total > 0 && (
                          <span className={`ml-0.5 text-[10px] px-1.5 py-0.5 rounded-full transition-colors duration-300 ${
                            isActive ? 'bg-[var(--color-bg)] text-[var(--color-muted-dark)]' : 'text-[var(--color-muted)]'
                          }`}>
