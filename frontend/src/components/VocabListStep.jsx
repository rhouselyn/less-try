...
function VocabListStep({ vocab, onClose, loading, t, currentFileId, sourceLang, pageSize = 50 }) {
  ...
  const renderPagination = () => {
    ...
    return (
      <div className="flex items-center justify-center gap-0.5 py-1.5 border-t-[var(--border-width)] border-[var(--border-color)] bg-[var(--color-bg)] shrink-0">
        <button
          onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
          disabled={currentPage <= 1}
          className={`p-1 rounded transition-colors ${currentPage <= 1 ? 'text-[var(--color-muted)] cursor-not-allowed opacity-30' : 'text-[var(--color-muted)] hover:text-[var(--color-dark)] hover:bg-[var(--color-bg)]'}`}
        >
          <ChevronLeft className="w-3.5 h-3.5" />
        </button>
        {pages.map((p, i) =>
          p === '...' ? (
            <span key={`dots-${i}`} className="text-[10px] text-[var(--color-muted)] px-0.5">…</span>
          ) : (
            <button
              key={p}
              ...
              className={`min-w-[22px] h-[22px] flex items-center justify-center text-[10px] rounded transition-colors ${
                currentPage === p
                  ? 'bg-[var(--color-highlight)] text-[var(--color-accent)] font-black uppercase'
                  : 'text-[var(--color-muted)] hover:text-[var(--color-dark)] hover:bg-[var(--color-bg)]'
              }`}
            >
              {p}
            </button>
        ...
        <button
          ...
          className={`p-1 rounded transition-colors ${currentPage >= totalPages ? 'text-[var(--color-muted)] cursor-not-allowed opacity-30' : 'text-[var(--color-muted)] hover:text-[var(--color-dark)] hover:bg-[var(--color-bg)]'}`}
        >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-overlay)]">
      <div className="bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-md)] shadow-[var(--shadow)] overflow-hidden flex flex-col max-w-2xl max-h-[85vh] w-full">
        <div className="px-5 py-3.5 border-b-[var(--border-width)] border-[var(--border-color)] bg-[var(--color-bg)]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-[var(--color-accent)]" />
              <h2 className="text-sm font-black uppercase font-display text-[var(--color-dark)]">{t.vocabList || '单词表'}</h2>
              <span className="badge-ochre text-xs px-2 py-0.5 rounded-full bg-[var(--color-highlight)] text-[var(--color-accent)]">
                {vocab.length}
              </span>
              {searchQuery && filteredVocab.length !== vocab.length && (
                <span className="text-xs text-[var(--color-muted)]">· {t.matched || '匹配'} {filteredVocab.length}</span>
              )}
            </div>
            <button
              onClick={onClose}
              className="p-1.5 text-[var(--color-muted)] hover:text-[var(--color-dark)] hover:bg-[var(--color-bg)] rounded-lg transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="px-4 py-2 border-b-[var(--border-width)] border-[var(--border-color)]">
          <div className="relative">
            ...
            <input
              ...
              className="input-warm w-full pl-9 pr-3 py-1.5 bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-lg text-[13px] text-[var(--color-dark)] placeholder:text-[var(--color-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-dark)] focus:border-[var(--border-color)] transition-all"
            />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <>
              {allLetterIndex.length > 1 && !searchQuery && (
                <div className="flex flex-col items-center gap-px py-1 border-r-[var(--border-width)] border-[var(--border-color)] bg-[var(--color-bg)] w-5 shrink-0 overflow-y-auto">
                    ...
                    {groupedVocab.map(([letter, words], groupIdx) => (
                      <div key={`group-${groupIdx}-${letter}`} id={`vocab-group-${letter}`}>
                        <motion.div
                          ...
                          className="sticky top-0 z-10 bg-[var(--color-bg)] px-4 py-1.5 border-b-[var(--border-width)] border-[var(--border-color)] mb-1"
                        >
                          <span className="text-xs font-black uppercase text-[var(--color-accent)] tracking-widest">{letter}</span>
                        </motion.div>
                            ...
                            return (
                              ...
                              >
                                <button
                                  ...
                                  className="w-full text-left px-4 py-2.5 flex items-center gap-2 hover:bg-[var(--color-highlight)] transition-colors group"
                                >
                                  <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap select-text">
                                    <span className="text-[14px] font-black uppercase text-[var(--color-dark)] tracking-tight shrink-0">
                                      {word.word}
                                    </span>
                                    {(enriched.ipa || word.ipa) && (
                                      <span className="text-[11px] text-[var(--color-muted)] ipa-font shrink-0">
                                        {(enriched.ipa || word.ipa).startsWith('/') ? (enriched.ipa || word.ipa) : `/${enriched.ipa || word.ipa}/`}
                                      </span>
                                    )}
                                    {(enriched.morphology || word.morphology) && (
                                      <span className="text-[10px] px-1.5 py-0.5 bg-[var(--color-bg)] text-[var(--color-muted-dark)] rounded font-medium tracking-wide shrink-0 border-[var(--border-width)] border-[var(--border-color)]">
                                        {enriched.morphology || word.morphology}
                                      </span>
                                    )}
                                    <span className="text-[12px] text-[var(--color-muted-dark)] truncate">
                                      {displayMeaning}
                                    </span>
                                  </div>
                                  <Volume2
                                    className="w-3.5 h-3.5 text-[var(--color-muted)] hover:text-[var(--color-accent)] shrink-0 transition-colors"
                                ...
                                <AnimatePresence>
                                  {isExpanded && (
                                    ...
                                    >
                                      <div className="px-4 pb-3.5 border-t-[var(--border-width)] border-[var(--border-color)]">
                                        {loadingWord === word.word ? (
                                          <div className="pt-4 flex flex-col items-center justify-center gap-3">
                                            <Loader2 className="w-5 h-5 animate-spin text-[var(--color-accent)]" />
                                            <p className="text-[12px] text-[var(--color-muted)]">{t.loadingWordDetails || '正在加载单词详情...'}</p>
                                          </div>
                                        ) : (() => {
                                          ...
                                          return hasDetail ? (
                                            ...
                                            <div className="pt-3 text-center text-[var(--color-muted)] text-[12px]">{t.noDetails || '暂无详情'}</div>
                                          )
                                        })()}
                                      </div>
                                    </motion.div>
