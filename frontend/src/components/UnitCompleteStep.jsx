    ...
    >
      {hasWrongItems && onSkipReview && (
        <motion.button
          ...
          className="absolute -top-2 left-0 z-10 flex items-center gap-1.5 btn-ghost text-xs border-[var(--border-width)] border-[var(--border-color)] hover:border-[var(--color-accent)]"
        >
          <X className="w-3 h-3" />
          {t.skipReview || '不想复习了'}
        </motion.button>
      )}
      <div className="bg-[var(--color-card)] border-[var(--border-width)] border-[var(--border-color)] rounded-[var(--radius-md)] p-12 shadow-[var(--shadow)] text-center">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 15, delay: 0.2 }}
          className="w-24 h-24 bg-[var(--color-highlight)] rounded-full border-[var(--border-width)] border-[var(--border-color)] flex items-center justify-center mx-auto mb-8"
        >
        ...
        <motion.h2
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3 }}
          className="text-3xl font-bold font-display text-[var(--color-dark)] mb-4"
        >
          🎉 {t.unitComplete || '单元完成！'}
        </motion.h2>
        ...
        <motion.p
          ...
          className="text-lg text-[var(--color-dark)] mb-2"
        >
        ...
        <motion.p
          ...
          className="text-base text-[var(--color-muted-dark)] mb-8"
        >
          {errorCount === 0
        ...
        {hasWrongItems && (
          <motion.div
            ...
            className="mb-8 p-4 bg-[var(--color-highlight)] border-[var(--border-width)] border-[var(--color-accent)] rounded-[var(--radius-sm)]"
          >
            <div className="flex items-center gap-2 justify-center mb-2">
              <RotateCcw className="w-4 h-4 text-[var(--color-accent)]" />
              <span className="text-[var(--color-dark)] font-medium">{t.wrongItemReview || '错题复习'}</span>
            </div>
        ...
        >
          ...
          <span className="text-[var(--color-muted-dark)] text-sm">
