    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-2">
          <motion.button
            onClick={onBack}
            className="flex items-center gap-2 text-umber-600 hover:text-umber-800 hover:bg-white px-3 py-1.5 rounded-lg transition-colors"
            whileTap={{ scale: 0.98 }}
          >
        ...
        <div className="flex items-center gap-3">
          {totalItemsInUnit > 0 && (
            <span className="text-sm text-umber-600 font-medium bg-canvas-100 px-3 py-1 rounded-full">{(t.stepProgress || '第 {0} / {1} 题').replace('{0}', stepInUnit).replace('{1}', totalItemsInUnit)}</span>
          )}
          {onOpenVocabList && (
            <motion.button
      ...
      <div className="bg-white border border-stone-400 rounded-3xl p-8 shadow-impasto">
        <div className="text-center mb-8">
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
            className="inline-flex items-center gap-2.5 px-5 py-2 bg-cadmium-100 text-cadmium-700 rounded-full text-base font-medium mb-5"
          >
          ...
          <div className="flex items-center justify-center gap-2">
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-xl text-umber-800 italic font-display"
            >
              {data.native_translation}
            </motion.p>
          </div>
        ...
        <div className="mb-8">
          <div className="p-5 border-3 border-dashed border-cadmium-300 rounded-2xl flex flex-wrap gap-3 bg-canvas-50 relative">
            <div className="flex flex-wrap gap-3 invisible" aria-hidden="true">
              {data.original_tokens.map((_, i) => (
                <span key={`ph-${i}`} className="px-5 py-2.5 rounded-full text-base font-medium">{data.original_tokens[i]}</span>
              ))}
              <span className="ml-auto p-2.5"><Lightbulb className="w-6 h-6" /></span>
            </div>
            <div className="absolute inset-0 p-5 flex flex-wrap gap-3 items-start content-start">
              {selectedTokens.length === 0 && (
                <span className="italic text-umber-500 text-base pointer-events-none">{t.tapToReconstruct || '按顺序点击下方词语还原句子'}</span>
              )}
                ...
                {selectedTokens.map((item, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`px-5 py-2.5 rounded-full text-base font-medium cursor-pointer select-none ${
                        answerChecked
                          ? isCorrect
                          ...
                          : 'bg-umber-700 text-white hover:bg-umber-800 shadow-md'
                      }`}
                    >
                      {item.token}
                    </motion.div>
              ...
              <motion.button
                ...
                className="ml-auto p-2.5 text-cadmium-500 hover:text-cadmium-700 hover:bg-cadmium-100 rounded-full transition-colors"
            ...
            {data.options.map((token, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className={`px-5 py-2.5 rounded-full text-base font-medium select-none ${
                    isSelected
                      ? 'pointer-events-none invisible'
                      : answerChecked
                        ? 'pointer-events-none bg-umber-700 text-white opacity-50'
                        : 'bg-umber-700 text-white hover:bg-umber-800 shadow-md'
                  }`}
                >
                  {token}
        ...
        {answerChecked && (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className={`font-semibold text-xl ${isCorrect ? 'text-teal-800' : 'text-vermilion-800'}`}>{isCorrect ? t.correct : t.incorrect}</span>
            </div>
            {!isCorrect && (
              <p className="text-umber-700 font-medium text-lg">
                {t.correctAnswer || '正确答案'}：{data.original_tokens.join(' ')}
              </p>
            )}
            {isCorrect && isLastExercise && (
              <p className="font-medium mt-4 text-xl text-teal-700">
        ...
        <div className="flex gap-4">
          {!answerChecked ? (
            <motion.button
              ...
              className="flex-1 py-4 btn-primary text-xl rounded-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {t.checkAnswer}
            </motion.button>
          ) : (
