...
test('Phase 1 units should display completed status with green color and checkmark', () => {
  ...
  expect(phase1CompletedUnit).toHaveClass('border-stone-300', 'bg-stone-50');
  
  // Check that incomplete current unit also has the same styling
  const phase2Units = screen.getAllByText('单元 2');
  const phase1IncompleteUnit = phase2Units[0].closest('button');
  expect(phase1IncompleteUnit).toHaveClass('border-stone-300');
});