...
class BSQDCCodesDataset(torch.utils.data.Dataset):
    def __init__(self, feature_dir, label_dir, num_samples=None):
        ...
        if num_samples is not None:
            self.feature_files = [f"{i}.npy" for i in range(num_samples)]
            self.label_files = [f"{i}.npy" for i in range(num_samples)]
        else:
...
def main(args):
    ...
    logger.info(f"Image size: {args.image_size}, Downsample size: {args.downsample_size}, Latent size: {latent_size}, Block size: {latent_size ** 2}")
    ...
    assert os.path.exists(feature_dir) and os.path.exists(label_dir), \
        f"please first run: python extract_codes_bsqdc.py ..."

    dataset = BSQDCCodesDataset(feature_dir, label_dir, num_samples=args.num_samples)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-samples", type=int, default=None, help="number of training samples (None for auto-detect from directory)")
