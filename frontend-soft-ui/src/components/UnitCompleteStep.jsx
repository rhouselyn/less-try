    ...
    >
      {hasWrongItems && onSkipReview && (
        <motion.button
          ...
          className="absolute -top-2 left-0 z-10 flex items-center gap-1.5 btn-ghost text-xs border border-theme-border hover:border-theme-border backdrop-blur-sm"
        >
          <X className="w-3 h-3" />
          {t.skipReview || '不想复习了'}
        </motion.button>
      )}
      <div className="bg-white border border-theme-border/60 rounded-3xl p-12 shadow-card text-center">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 15, delay: 0.2 }}
          className="w-24 h-24 bg-theme-bg-subtle rounded-full flex items-center justify-center mx-auto mb-8"
        >
        ...
        <motion.p
          ...
          className="text-lg text-theme-text-secondary mb-2"
        >
        ...
        <motion.p
          ...
          className="text-base text-theme-text-secondary mb-8"
        >
          {errorCount === 0
        ...
        {hasWrongItems && (
          <motion.div
            ...
            className="mb-8 p-4 bg-theme-bg-subtle border border-soft-500/20 rounded-2xl"
          >
            <div className="flex items-center gap-2 justify-center mb-2">
              <RotateCcw className="w-4 h-4 text-theme-primary" />
              <span className="text-theme-text font-medium">{t.wrongItemReview || '错题复习'}</span>
            </div>
        ...
        >
          {hasWrongItems ? (
            <motion.button
              ...
              onClick={onReview}
              className="btn-primary px-6 py-3 rounded-2xl font-semibold shadow-btn hover:-translate-y-0.5 hover:shadow-card-hover transition-all duration-200 text-lg flex items-center justify-center gap-2"
            >
