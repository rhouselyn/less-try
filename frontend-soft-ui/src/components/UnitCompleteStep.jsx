    ...
    >
      {hasWrongItems && onSkipReview && (
        <motion.button
          ...
          className="absolute -top-2 left-0 z-10 flex items-center gap-1.5 btn-ghost text-xs border border-slate-200 hover:border-slate-300 backdrop-blur-sm"
        >
          <X className="w-3 h-3" />
          {t.skipReview || '不想复习了'}
        </motion.button>
      )}
      <div className="bg-white border border-slate-200/60 rounded-3xl p-12 shadow-soft-md text-center">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 15, delay: 0.2 }}
          className="w-24 h-24 bg-soft-50 rounded-full flex items-center justify-center mx-auto mb-8"
        >
          <Trophy className="w-12 h-12 text-soft-500" />
        </motion.div>
        ...
        {hasWrongItems && (
          <motion.div
            ...
            className="mb-8 p-4 bg-soft-50 border border-soft-500/20 rounded-2xl"
          >
            <div className="flex items-center gap-2 justify-center mb-2">
              <RotateCcw className="w-4 h-4 text-soft-500" />
              <span className="text-slate-700 font-medium">{t.wrongItemReview || '错题复习'}</span>
            </div>
        ...
        >
          <Sparkles className="w-5 h-5 text-soft-500" />
        ...
        >
          {hasWrongItems ? (
            <motion.button
              ...
              onClick={onReview}
              className="btn-primary px-6 py-3 rounded-2xl font-semibold shadow-soft-lg hover:-translate-y-0.5 hover:shadow-soft-xl transition-all duration-200 text-lg flex items-center justify-center gap-2"
            >
