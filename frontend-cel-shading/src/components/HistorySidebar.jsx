...
function ContextMenu({ x, y, onRename, onDelete, onClose, t }) {
  ...
  return (
    <motion.div
      ...
      className="fixed z-[9999] bg-white border-[3px] border-[#1a1a2e] rounded-xl shadow-cel-lg py-1.5 min-w-[160px] overflow-hidden"
    >
      <button
        onClick={() => { onRename(); onClose() }}
        className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[13px] text-[#1a1a2e] hover:bg-[#f4a261]/20 transition-colors"
      >
        <Pencil className="w-3.5 h-3.5 text-[#4a4a6a]" />
        {t.rename || '重命名'}
      </button>
      <div className="mx-2.5 my-0.5 border-t-[3px] border-[#1a1a2e]" />
      <button
        onClick={() => { onDelete(); onClose() }}
        className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[13px] text-[#e76f8b] hover:bg-[#fde8e8] transition-colors"
      >
...
function ProgressBadge({ progress }) {
  ...
  return (
    <span className={`w-[32px] text-right text-[10px] font-light tabular-nums flex-shrink-0 ${done ? 'text-[#2a9d8f]' : 'text-[#e76f8b]'}`}>
...
function HistoryItem({ record, isRenaming, renameValue, onRenameStart, onRenameConfirm, onRenameCancel, onRenameChange, onNavigate, onMenuOpen, t }) {
      ...
      <div className="px-2.5 py-1.5" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-1">
          <input
            ...
            className="flex-1 text-[13px] px-2 py-1 border-[3px] border-[#1a1a2e] rounded-md focus:outline-none focus:ring-1.5 focus:ring-[#1a1a2e] bg-white text-[#1a1a2e]"
          />
        </div>
      </div>
    )
  ...
  return (
    <div
      className="group flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer hover:bg-[#f4a261]/20 transition-colors mx-2 bg-white"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[13px] text-[#1a1a2e] truncate leading-snug">
          {record.title}
        </div>
      </div>
      <button
        ...
        className="opacity-0 group-hover:opacity-100 p-1 rounded-md hover:bg-[#f4a261]/20 text-[#4a4a6a] hover:text-[#1a1a2e] transition-all flex-shrink-0"
      >
...
function RecentItem({ record, onNavigate }) {
  return (
    <button
      onClick={() => onNavigate(record.file_id, record.source_lang, record.target_lang, record.title)}
      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#f4a261]/20 transition-colors text-left"
    >
      <ProgressBadge progress={record.progress} />
      <span className="text-[13px] text-[#1a1a2e] truncate flex-1">{record.title}</span>
      <span className="text-[10px] font-black uppercase text-[#4a4a6a] flex-shrink-0">{(record.source_lang || '?').substring(0, 2)}</span>
    </button>
  )
}
      ...
      <div className="flex h-full">
        <AnimatePresence>
          {expanded && (
            <motion.div
              ...
              className="h-full overflow-hidden flex flex-col bg-[#f0e6d3] border-r-[3px] border-[#1a1a2e]"
              style={{ minWidth: 0 }}
            >
              <div className="px-3 pt-4 pb-2 flex items-center justify-between flex-shrink-0">
                <span className="font-display text-[11px] font-black uppercase text-[#1a1a2e] tracking-wider">
                  {t.historyTitle || '学习记录'}
                </span>
                <button
                  onClick={() => setExpanded(false)}
                  className="p-1 rounded-md hover:bg-[#f4a261]/20 text-[#4a4a6a] hover:text-[#1a1a2e] transition-colors"
                >
              ...
              <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin">
                {records.length === 0 && (
                  <div className="px-4 py-12 text-center">
                    ...
                    <div className="text-[13px] text-[#4a4a6a]">
                      {t.noHistory || '暂无学习记录'}
                    </div>
                  </div>
                )}
                ...
                {records.length > 0 && (
                  <div className="mb-2">
                    <div className="px-4 py-1.5 mt-1">
                      <span className="text-[11px] font-medium text-[#4a4a6a] tracking-wide">
                        {t.recent || '最近'}
                      </span>
                    </div>
                    ...
                    {hasMoreRecent && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1 px-3 py-1.5 text-[11px] text-[#4a4a6a] hover:text-[#1a1a2e] transition-colors"
                      >
                        {recentExpanded ? (
                          <>
                    ...
                    <div className="mx-4 my-2 border-t-[3px] border-[#1a1a2e]" />
                  </div>
                )}
                ...
                {Object.entries(grouped).map(([lang, items], langIdx) => (
                  <div key={lang} className="mb-1">
                    <div className="flex items-center gap-1.5 px-4 py-1.5 mt-1">
                      ...
                      <span className="text-[10px] text-[#4a4a6a]">{items.length}</span>
                      <button
                        ...
                        className={`ml-auto p-1 rounded-md transition-all badge-ochre ${
                          activeWordListLang === lang
                            ? SIDEBAR_COLORS[langIdx % SIDEBAR_COLORS.length] + ' text-white shadow-cel-sm'
              ...
              <div className="px-3 py-2 border-t-[3px] border-[#1a1a2e] flex-shrink-0">
        ...
        {!expanded && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex-shrink-0 flex flex-col items-center pt-4 pb-2 gap-1 bg-[#f0e6d3] border-r-[3px] border-[#1a1a2e]"
            style={{ width: 48 }}
          >
            <button
              onClick={() => setExpanded(true)}
              className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[#f4a261]/20 text-[#4a4a6a] hover:text-[#1a1a2e] transition-colors"
              title={t.historyTitle || '学习记录'}
            >
              <PanelLeftOpen className="w-4.5 h-4.5" />
            </button>

            <div className="w-6 border-t-[3px] border-[#1a1a2e] my-1" />
            ...
            {langKeys.map((lang, idx) => (
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-lg text-[13px] font-bold transition-all ${
                  activeWordListLang === lang
                    ? SIDEBAR_COLORS[idx % SIDEBAR_COLORS.length] + ' text-white shadow-cel border-[3px] border-[#1a1a2e]'
                    : 'bg-[#f0e6d3] text-[#2d2d4a] hover:bg-[#f4a261]/20 hover:text-[#1a1a2e] border-[3px] border-[#1a1a2e]'
                }`}
            ...
            {langKeys.flatMap(lang =>
              grouped[lang].slice(0, 3).map((record) => (
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[#f4a261]/20 text-[10px] font-medium text-[#4a4a6a] hover:text-[#1a1a2e] transition-colors"
                  title={record.title}
                >
      ...
      <AnimatePresence>
        {deleteConfirm.open && (
          <motion.div
            ...
            className="fixed inset-0 z-[10000] flex items-center justify-center bg-[#1a1a2e]/40 backdrop-blur-sm"
          ...
          >
            <motion.div
              ...
              className="bg-white border-[3px] border-[#1a1a2e] rounded-2xl shadow-cel-xl p-6 max-w-sm w-full mx-4"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-[#fde8e8] flex items-center justify-center flex-shrink-0">
                  <Trash2 className="w-5 h-5 text-[#e76f8b]" />
                </div>
                <div>
                  <h3 className="font-display text-[15px] font-black uppercase text-[#1a1a2e]">{t.confirmDelete || '确认删除'}</h3>
                  <p className="text-[13px] text-[#2d2d4a] mt-0.5 line-clamp-2">{deleteConfirm.title}</p>
                </div>
              </div>
              <p className="text-[13px] text-[#2d2d4a] mb-5 pl-[52px]">{t.deleteCannotUndo || '删除后不可恢复，确定要删除吗？'}</p>
              <div className="flex gap-2.5 justify-end">
                <button
                  onClick={handleDeleteCancel}
                  className="btn-secondary px-4 py-2 text-[13px] rounded-xl border-[3px] border-[#1a1a2e] text-[#1a1a2e] hover:bg-[#f0e6d3] transition-colors"
                >
                  {t.cancel || '取消'}
                </button>
                <button
                  onClick={handleDeleteConfirm}
                  className="px-4 py-2 text-[13px] rounded-xl bg-[#e76f8b] hover:bg-[#e76f8b] text-white font-black uppercase transition-colors border-[3px] border-[#1a1a2e] shadow-cel-sm"
                >
